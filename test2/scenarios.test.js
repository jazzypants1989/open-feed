// The scenarios of GOALS.md, end to end over src2/: the divorce, Grandma onboards, two hubs one
...
import { signFile } from '../src2/file.js';
import { signProfile } from '../src2/profile.js';
import { rotation, restore, vouched } from '../src2/profile.js';
import { seal, open, carrierOf, newReadingKey, spokenCode } from '../src2/index.js';
import * as views from '../src2/views.js';
import { memIo, readerOver, person, list, members, claim } from './helpers/site2.js';

test('the divorce: he cannot post as her, read what is sealed past him, alter or backdate her words, keep her, or keep her identity', async () => {
  ...
  await pub.publish(1, { at: '2026-08-01T00:00:00Z', sealed: seal({ content: { text: 'I am leaving him on Friday' }, audience, carrier: carrierOf(alice.key.x, 1) }) });
  ...
  // He cannot read it, and he cannot re-parent her sealed words under his own name.
  const sealed = (await reader.read({ learned: alice.key.x, at: AT })).posts.get(1).sealed;
  assert.equal(open(sealed, newReadingKey().privateKey, carrierOf(alice.key.x, 1)), null);
  assert.equal(open(sealed, mumRead.privateKey, carrierOf(ex.key.x, 1)), null, 'lifted into his post it does not open');
  assert.equal(open(sealed, mumRead.privateKey, carrierOf(alice.key.x, 1)).text, 'I am leaving him on Friday');
  ...
  assert.equal((await reader.read({ learned: alice.key.x, at: AT, pin: momPin })).why, 'post 2 is not what the head lists');
  hub.store.set('alice/posts/2', signFile({ n: 2, at: '2026-08-02T00:00:00Z', text: 'a public post' }, alice.key));
  assert.equal((await reader.read({ learned: alice.key.x, at: AT, pin: momPin })).verdict, 'ok', '§3.2: the identical body re-signed IS the post — the address is the hash of the body');
  hub.store.set('alice/posts/2', signFile({ n: 2, at: '2026-08-02T00:00:00Z', text: 'a public post.' }, alice.key));
  assert.equal((await reader.read({ learned: alice.key.x, at: AT, pin: momPin })).why, 'post 2 is not what the head lists', 'one changed byte, even under her own key, is not');
  hub.store.set('alice/posts/2', signFile({ n: 2, at: '2026-08-02T00:00:00Z', text: 'a public post' }, alice.key));

  // He steals her key; she restores; the head is re-signed and what he withdrew comes back.
  const K2 = person('k2');
  const thiefPub = createPublisher({ io, key: alice.key, at: AT });           // him, with her stolen key
  await thiefPub.withdraw(2);
  const restored = signProfile({ genesis: alice.key.x, pseq: 2, name: 'alice', chain: [{ key: alice.key.x }, restore(alice.key, K2.key, members(mum, sis), REC)], recovery: REC, locations: [AT], read: aliceRead.x }, K2.key);
  ...
  // His fork from the stolen key, after the fact, loses to the court — and her mother can check
  // the six words of the branch she was handed out of band.
  const fork = signProfile({ genesis: alice.key.x, pseq: 3, chain: [{ key: alice.key.x }, rotation(alice.key, ex.key, REC)], recovery: REC, locations: [AT] }, ex.key);
  hub.store.set('alice/profile', fork);
  await createPublisher({ io, key: ex.key, at: AT }).resignHead();
  assert.equal((await reader.read({ learned: alice.key.x, at: AT, pin: after.pin })).why, 'serves a branch the court rejected');
  ...
  for (const [path, bytes] of pub.copy) if (path !== '/head' && path !== '/profile') await homeIo.put(`https://alice.example/alice${path}`, bytes);
  const pubHome = createPublisher({ io: homeIo, key: K2.key, at: 'https://alice.example/alice' });
  await homeIo.put('https://alice.example/alice/profile', signProfile({ genesis: alice.key.x, pseq: 3, name: 'alice', chain: [{ key: alice.key.x }, restore(alice.key, K2.key, members(mum, sis), REC)], recovery: REC, locations: ['https://alice.example/alice'] }, K2.key));
  await pubHome.amendHead((h) => ({ entries: [[1, momPin.live.get(1)], [2, momPin.live.get(2)]], hseq: 9, top: 2 }));
...
test('Grandma onboards: an app, a name, no key ever shown; back by calling her daughter', async () => {
  ...
  hub.store.set('grandma/profile', signProfile({ genesis: grandma.key.x, pseq: 2, name: 'grandma', chain: [{ key: grandma.key.x }, restore(grandma.key, newPhone.key, members(daughter), REC)], recovery: REC, locations: [AT] }, newPhone.key));
  await createPublisher({ io, key: newPhone.key, at: AT }).resignHead();
...
test('two hubs, one thread: a sealed post, a sealed reply and a reaction cross hubs with no access control anywhere', async () => {
  ...
  await mpub.publish(1, { at: '2026-08-10T09:00:00Z', sealed: seal({ content: { text: 'the scan came back clear' }, audience: fam, carrier: carrierOf(mom.key.x, 1) }) });
  const momRead = await reader.read({ learned: mom.key.x, at: at.mom, pin: seen.get(mom.key.x) });
  seen.set(mom.key.x, momRead.pin);
  const inner = open(momRead.posts.get(1).sealed, reads.jesse.privateKey, carrierOf(mom.key.x, 1));
  assert.equal(inner.text, 'the scan came back clear');
  // Jesse replies sealed, resolving the member he does not follow from the audience entry (§7.5).
  const unknown = inner.audience.find((a) => !seen.has(a.key));
  assert.equal(unknown, undefined, 'jesse holds pins for all three here');
  await jpub.publish(1, { at: '2026-08-10T11:00:00Z', sealed: seal({ content: { rel: 'reply', target: { key: mom.key.x, n: 1, hash: momRead.pin.live.get(1), loc: at.mom }, text: 'best news all year' }, audience: inner.audience, carrier: carrierOf(jesse.key.x, 1) }) });
  const jRead = await reader.read({ learned: jesse.key.x, at: at.jesse, pin: seen.get(jesse.key.x) });
  const reply = open(jRead.posts.get(1).sealed, reads.sis.privateKey, carrierOf(jesse.key.x, 1));
  ...
  assert.equal(open(momRead.posts.get(1).sealed, newReadingKey().privateKey, carrierOf(mom.key.x, 1)), null);
...
test('the domain goes: everyone relocates, nobody\'s identity changes, readers find them from the locations they hold', async () => {
  ...
  await pub.updateProfile({ genesis: a.key.x, pseq: 2, name: 'alice', chain: [{ key: a.key.x }], recovery: list(0), locations: [AT, NEW] });
  ...
  await pubNew.claim({ genesis: a.key.x, pseq: 3, name: 'alice', chain: [{ key: a.key.x }], recovery: list(0), locations: [NEW] });
  for (const [path, bytes] of pub.copy) if (path.startsWith('/posts/')) await io.put(`${NEW}${path}`, bytes);
  await pubNew.amendHead((h) => ({ ...h, entries: [[1, pin.live.get(1)]], top: 1 }));
  old.store.clear();
...
test('the stranger: a public journal reaches a plain feed reader through the generated views, which are never evidence', async () => {
  ...
  await pub.publish(3, { at: '2026-08-03T00:00:00Z', sealed: { epk: 'x', slots: [], ct: 'y' } });
  const r = await readerOver(io).read({ learned: a.key.x, at: AT });
  const feed = JSON.parse(views.jsonFeed(r, AT));
  assert.equal(feed.items.length, 2, 'sealed posts are omitted from views');
  ...
  assert.ok(views.hcard(r, AT).includes(`#${a.key.x}`), 'the h-card link carries the genesis key in its fragment');
