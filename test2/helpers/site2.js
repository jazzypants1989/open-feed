// The test origin for spec-2: `src2/hub.js` behind a TLS socket with the hand-encoded certificate
// (§3.1 of the old spec made HTTPS part of the identity; §10 of this one makes it the only
// transport), and an in-memory io over `hub.handle` for the tests that need no socket at all.
import { selfSignedCertificate } from '../../test/helpers/tls.js';
import { isPublicOrLoopbackAddress } from '../../src2/addresses.js';
import { createFetcher } from '../../src2/fetch.js';
import { createHub, listen } from '../../src2/hub.js';
import { createReader } from '../../src2/reader.js';
import { createPublisher } from '../../src2/publish.js';
import * as file from '../../src2/file.js';
import * as profile from '../../src2/profile.js';
...
/** Claim a name on a hub for `p`: profile at pseq 1, an empty head. Returns the publisher. */
export async function claim(io, p, at, { recovery, read, extra = {} } = {}) {
  const pub = createPublisher({ io, key: p.key, at });
  await pub.claim({ genesis: p.key.x, pseq: 1, ...(p.name ? { name: p.name } : {}), chain: [{ key: p.key.x }], recovery: recovery ?? { k: 0, leaves: [] }, locations: [at], ...(read ? { read } : {}), ...extra });
  return pub;
}
