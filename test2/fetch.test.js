// §10 — the fetch layer over a real TLS socket: HTTPS only, the resolved-address guard, literals,
// same-origin redirects, caps, and PUT with If-Match. The certificate is hand-encoded (helpers).
import test from 'node:test';
import assert from 'node:assert/strict';
import http from 'node:http';
import { isPublicAddress } from '../src2/addresses.js';
import { createFetcher, guardedLookup, FetchError } from '../src2/fetch.js';
import { tlsHub, consumerFetcher, HOSTNAME, person, list, claim, memIo } from './helpers/site2.js';
import { createReader } from '../src2/reader.js';

test('§10 the whole read path works over TLS with a validated (pinned) certificate', async (t) => {
  ...
  const cur = await f.get(`${url}/alice/head`);
  assert.ok(cur.etag);
  const put = await f.put(`${url}/alice/head`, cur.bytes, { ifMatch: cur.etag });
  assert.equal(put.status, 200);
  assert.equal((await f.put(`${url}/alice/head`, cur.bytes, { ifMatch: '"stale"' })).status, 412);
  assert.equal(await f.get(`${url}/alice/posts/1`), null, 'a 404 is null, not an error');
});

test('§10 HTTPS only, and non-public addresses refused as literals and through DNS', async () => {
  const f = consumerFetcher();
...
test('§10 redirects: never cross-origin, at most five, each hop re-checked', async (t) => {
  let hops = 0;
  ...
  await assert.rejects(g.get(`${url}/meta`), (e) => e.code === 'blocked_address', 'each hop is re-checked');
});

test('§10 the byte cap stops the transfer; a failure is a FetchError, never a verdict', async (t) => {
  const server = http.createServer((req, res) => { res.writeHead(200); res.end(Buffer.alloc(64 * 1024)); });
  await new Promise((r) => server.listen(0, '127.0.0.1', r));
  t.after(() => server.close());
  const f = createFetcher({ protocols: ['http:'], isAddressAllowed: () => true, maxBytes: { profile: 1024, head: 1024, post: 1024, media: 1024 } });
