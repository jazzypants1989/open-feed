// §3 — the file format, addresses, no canonicalization, JSON hygiene.
import test from 'node:test';
import assert from 'node:assert/strict';
import { signFile, verifyFile, address, decodeStrict, parseStrict, parseBody, newSigningKey, sha256, splitFile } from '../src2/file.js';

const k = newSigningKey();
const ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';

test('§3.1 a file is body, one newline, an 86-character signature over the body', () => {
...
test('§3.1 the signature line must re-encode to itself — a second spelling of the same bytes is refused', () => {
...
test('§3.2 the address is the hash of the body, never of the file', () => {
  const f = signFile({ a: 1 }, k);
  assert.equal(address(f), sha256(splitFile(f).body));
  assert.notEqual(address(f), sha256(f));
});

test('§3.3 a host that pretty-prints, reorders, or appends a newline makes the file read as forged', () => {
...
test('§3.4 the parser rejects duplicate members, __proto__, integers past 2^53, lone surrogates, raw controls', () => {
...
test('§3.1 a body with a BOM, a newline, invalid UTF-8, or a non-object is not a body', () => {
...
test('§3.5 unknown members survive: they are inside the signature and the verifier hands them back', () => {
