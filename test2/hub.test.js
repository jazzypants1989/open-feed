// §9 — the publish interface as the pure handler: CAS, verified writes, create-once, the owner's
// reclaim, the photo twin, CORS, views, and §9.8's collection.
import test from 'node:test';
import assert from 'node:assert/strict';
import { createHub } from '../src2/hub.js';
import { signFile, address, sha256 } from '../src2/file.js';
import { rotation, signProfile } from '../src2/profile.js';
import { memIo, person, list, claim, readerOver } from './helpers/site2.js';
...
test('§9.1 compare-and-swap: a stale tag is 412, and an overwrite with no If-Match at all is refused', async () => {
  const s = await scene();
  const etag = get(s.hub, '/alice/head').headers.etag;
  const K = s.alice.key;
  const stale = put(s.hub, '/alice/head', s.hub.store.get('alice/head'), '"stale"');
  assert.equal(stale.status, 412);
  assert.equal(stale.headers.etag, etag, 'the 412 carries the tag the hub is serving');
  assert.equal(put(s.hub, '/alice/head', s.hub.store.get('alice/head')).status, 412, 'no If-Match on an existing file');
});

test('§9.4 a hub that accepts writes verifies: a stranger with the public tag cannot clobber the profile or the head', async () => {
  const s = await scene();
  const S = person('stranger');
  const tag = get(s.hub, '/alice/profile').headers.etag;
  const forged = signProfile({ genesis: s.alice.key.x, pseq: 9, chain: [{ key: s.alice.key.x }], recovery: s.REC, locations: [] }, S.key);
  assert.equal(put(s.hub, '/alice/profile', forged, tag).status, 403);
  const otherGenesis = signProfile({ genesis: S.key.x, pseq: 9, chain: [{ key: S.key.x }], recovery: s.REC, locations: [] }, S.key);
  assert.equal(put(s.hub, '/alice/profile', otherGenesis, tag).status, 409, 'the name is taken, by someone else');
  const rollback = signProfile({ genesis: s.alice.key.x, pseq: 1, chain: [{ key: s.alice.key.x }], recovery: s.REC, locations: [] }, s.alice.key);
  assert.equal(put(s.hub, '/alice/profile', rollback, tag).status, 409, 'pseq must advance');
  const htag = get(s.hub, '/alice/head').headers.etag;
  assert.equal(put(s.hub, '/alice/head', signFile({ entries: [], hseq: 99, top: 0 }, S.key), htag).status, 403);
  // The honest case the checks must not break: a real rotation, then the head under the new key.
  const K2 = person('k2');
  const rotated = signProfile({ genesis: s.alice.key.x, pseq: 2, name: 'alice', chain: [{ key: s.alice.key.x }, rotation(s.alice.key, K2.key, s.REC)], recovery: s.REC, locations: [s.AT] }, K2.key);
  assert.equal(put(s.hub, '/alice/profile', rotated, tag).status, 200);
  assert.equal(put(s.hub, '/alice/head', signFile({ entries: [], hseq: 99, top: 3 }, s.alice.key), htag).status, 403, 'the old key no longer signs the head');
  assert.equal(put(s.hub, '/alice/head', signFile({ entries: [], hseq: 99, top: 3 }, K2.key), htag).status, 200);
});

test('§9.2 / §9.5 create-once, and the owner\'s reclaim under the ruled rule', async () => {
  const s = await scene();
  // A rotated-out key: the chain is [G, K2], the current key is K2, posts 1–3 are G-signed and listed.
  const K2 = person('k2');
  const tag = get(s.hub, '/alice/profile').headers.etag;
  put(s.hub, '/alice/profile', signProfile({ genesis: s.alice.key.x, pseq: 2, name: 'alice', chain: [{ key: s.alice.key.x }, rotation(s.alice.key, K2.key, s.REC)], recovery: s.REC, locations: [s.AT] }, K2.key), tag);
  ...
  // A file declaring another number is nobody's file for this one (§6.1: half of the reclaim rule).
...
test('§9.6 the photo twin: junk at her hash is replaced by bytes that match, and never the reverse', async () => {
  const s = await scene();
  const photo = Buffer.from('a tiny photo'), h = sha256(photo);
  ...
  assert.equal(put(s.hub, `/alice/media/${h}`, photo).status, 200, 'replaced by the bytes that hash to the name');
  assert.equal(put(s.hub, `/alice/media/${h}`, Buffer.from('junk')).status, 409);
  assert.equal(put(s.hub, `/alice/media/${h}`, photo).status, 409, 'the same bytes again is a collision, not a write');
});

test('§9.7 CORS on everything, and the preflight a browser publisher needs', async () => {
  const s = await scene();
  ...
  const pre = s.hub.handle({ method: 'OPTIONS', path: '/alice/head' });
...
test('§12 views are unsigned overwritable files at conventional paths, and never evidence', async () => {
  const s = await scene();
...
test('§9.8 a hub may drop what the head does not list, and nothing the head lists', async () => {
  const s = await scene();
