...
| `tracelife-exp.js` | handoff §2.A | **2026-08-21.** If the append-only head wins, which compaction policy: how many *days* a withdrawn post's lines stay in the file a late reader downloads, against what compaction costs every reader. The lines are ~6% of the head at either scale, so compaction is a privacy decision and not a size one; monthly caps the trace at 30 days and costs 108× the egress of never compacting |
| `hashwidth-exp.js` | handoff §2.B | **2026-08-21.** A 16-byte hash in the head: a collision search with teeth (4,783 found at 3 bytes, 0 at 16 over 400k hashes), the second-preimage work a stolen key faces (2^128 either way), and the one thing the prefix really costs — the author's own equivocation drops to 2^64, staged at 4 bytes with a real found collision and caught only because a reply's target carries all 32 bytes |

| `freezehead-exp.js` | final §2.F (RULINGS §12.7) | **2026-08-21.** The unverifiable-head freeze against the unchanged weekend reader under four observers: it is splitview's per-reader cap with the reader-identification cost removed, never moves a pin, leaves a note ordinary withholding does not, tells every cold reader `host`, and breaks on the social path after one re-fetch; a cold reader mid-rotation is told `host` under either write order, closed only by a reader-side retry |
| `views-exp.js` | final §2.G | **2026-08-21.** JSON Feed 1.1, Atom and h-card generated from the head and posts, consumed by three independent no-library consumers; nothing signed; a post edited inside the feed passes the feed consumer and the same edit as a file is caught by the unchanged reader; id = `urn:openfeed:<genesis>:<n>`, sealed posts omitted; **the profile has no display name** |
| `headbytes-exp.js` | final §2.I | **2026-08-21.** Real signed heads at 10–100,000 entries, 0/5/20% withdrawn, with and without a rewrite, against the arithmetic tracelife/headage/headrange assumed: worst ratio 0.9983; every ruled TB/yr row and the ~6% figure unchanged; a pending entry is +10 B |
