// spoken-gate: the six-word code of §4.1 — what it distinguishes, and what it costs to grind.
// §4.1 derives it from the GENESIS key and offers it "for the moment a person is shown two
// versions of one identity". Every §4.6 contest is two versions of one identity, and they share
// the genesis. And §4.1 says "at 40 bits a laptop core finds a colliding key in under a second".
// Finding A7 and text defect C1 of the 2026-08-23 review.
// Kill criteria: two branches of one identity speaking different codes under §4.1 as written;
...
// §4.1 verbatim: HKDF-SHA256(ikm = key bytes, salt = "", info = "openfeed/v1/spoken", 9 bytes),
...
console.log('\n1. A contest: Alice\'s branch and the thief\'s share the genesis.\n');
const G = pub.newKey(), K2 = pub.newKey(), T = pub.newKey();
const REC = pub.commit(1, [{ key: pub.newKey(), salt: 's' }]);
const alice = { genesis: G.x, chain: [{ key: G.x }, pub.rotation(G, K2, REC)] };
const thief = { genesis: G.x, chain: [{ key: G.x }, pub.rotation(G, T, REC)] };
const asWritten = [spoken(alice.genesis), spoken(thief.genesis)];
...
claim('a reader handed the current key out of band can pick the branch that contains it — the exit §4.6 lacks', alice.chain.some((h) => h.key === K2.x) && !thief.chain.some((h) => h.key === K2.x));
...
claim(`40 bits on one laptop core is ${fmt(row[40][0])}, not "under a second" — §4.1's number is wrong by ${Math.round(Math.log10(row[40][0]))} orders of magnitude`, row[40][0] > 3600 * 24);
