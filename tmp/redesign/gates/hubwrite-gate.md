...
**Question.** §9.5: "A hub MAY check nothing on the ordinary path." §9.4 asks a later profile
write for the same genesis and a higher `pseq`; §9.1's entity tag is what any GET returns; §13's
...
with her genesis, `pseq` 2, the right shape and his own signature; (2) her real bytes with
`pseq` bumped — the signature no longer matches; (3) a head signed by nobody. Each with the
`If-Match` a GET gave him. Then Alice's one-PUT repair, then the honest case the repair must
survive: a real rotation and the head re-signed under the new key. Under both knob settings.

**Numbers** (stale if §9.4, §9.5 or the reader's profile step change):
...
| her bytes, pseq bumped | **200** → `identity` | 403, `ok` |
| a head signed by nobody | **200** → cold `host: the head is not signed…`, pinned `ok` with a note | 403, `ok` |
| Alice writes herself back | 200 — and his next PUT is 200 again | — |
| her real rotation + re-signed head | 200 · ok · cold `ok` | 200 · ok · cold `ok` |
...
rotation or an honest head. **Not triggered.**
...
**Verdict.** §9.5's own principle — check nothing on the ordinary path, never ignore a collision —
applied to the profile and the head, where every overwrite *is* a collision: a hub that accepts
writes MUST refuse a profile whose chain does not walk or whose signature fails under its tip,
and a head not signed by the key the held profile ends on. Add it to §13's hub list, and split
that list: a *serving* hub (exact bytes, CORS — static hosting qualifies) from one that accepts
writes. Consequence for §4.5: a rotation writes the profile before the head.

**Run:** `node tmp/redesign/gates/hubwrite-gate.js`
