// media-gate: the fourth file kind, end to end. RULINGS §11.6 puts photos in the head "so retention
// is one rule and reaches sealed posts"; a blob is the only unsigned file. It had never been listed,
// fetched, withheld or swapped in any gate, and the head's entry shape had no room for it. Here the
...
// Kill criteria: a withheld or swapped blob the reader does not catch; a new photo a pinned reader
// calls an insertion; a griefer who can make the author's readers accuse her host; an entry wider
// than ruling 6's ~55 bytes; a photo's reference visible beside a sealed post.
...
class Hub {
  ...
  handle(method, url, b, ifMatch) {
    ...
    if (m[2] === 'head' || m[2] === 'profile') { if (this.tag(key) !== ifMatch) return { status: 412 }; this.files.set(key, b); return { status: 200, etag: this.tag(key) }; }
    if (this.files.has(key)) {
      // A blob is content-addressed: a file at a hash it does not hash to is nobody's, and may be
      // replaced by one that does. The numbered-file analogue is §12.5's reclaim rule.
...
const photo = crypto.randomBytes(20_000), hP = sha(photo);
const hubs = [];
...
// 1. a photo published and a post referencing it; the pinned reader sees a new photo, not an insertion
const s1 = await fresh();
const headBefore = (await s1.net.get(`${AT}/head`)).length;
await pub.publishMedia(s1.net, AT, A, photo);
const headAfter = (await s1.net.get(`${AT}/head`)).length;
...
// 3. a post referencing a hash the head does not list: the photo is simply not there
...
const linesAfterRewrite = JSON.parse((await s1.net.get(`${AT}/head`)).toString('utf8').split('\n')[0]).entries;
...
// 6. a second photo survives the rewrite listed by hash alone
...
// 7. the griefer: junk at her photo's hash before she puts it, on a hub that checks content on a
...
const a2 = await s2.net.put(`${AT}/media/${hP}`, photo), a3 = await s3.net.put(`${AT}/media/${hP}`, photo);
for (const s of [s2, s3]) await pub.amendHead(s.net, AT, A, (h) => ({ ...h, entries: [...h.entries, [hP]] }));
const griefChecked = await s2.see(), griefUnchecked = await s3.see();

// 8. a sealed post's photo: the bytes are ciphertext, the hash is listed, the reference is inside
//    the envelope — the post in the clear names no photo.
const s4 = await fresh();
const cipher = crypto.randomBytes(20_000), hC = sha(cipher);
await pub.publishMedia(s4.net, AT, A, cipher);
await pub.publish(s4.net, AT, A, 3, { at: '2026-08-02', sealed: { slots: ['…'], ct: 'opaque, and {media:[hC]} is in here' } });
const sealed = await s4.see();
...
console.log(`    a photo listed: the head grows by ${headAfter - headBefore} B; pinned ${listed.verdict} [${listed.note}], ${listed.media?.size ?? 0} photo, bytes ${listed.media?.get(hP)?.equals(photo) ? 'match' : 'DIFFER'}; cold ${coldListed.verdict}`);
console.log(`    withheld: ${withheld.verdict} (${withheld.why});  swapped: ${swapped.verdict} (${swapped.why})`);
console.log(`    a post naming an unlisted hash: ${unlisted.verdict} [${unlisted.note}], post 4 present ${unlisted.posts?.has(4)}, photo present ${unlisted.media?.has(stray)}`);
...
console.log(`    second photo, second rewrite: ${twoRewrites.verdict} [${twoRewrites.note}], photos ${[...(twoRewrites.media?.keys() ?? [])].map((k) => k.slice(0, 8)).join(',')}`);
...
console.log(`    sealed post: ${sealed.verdict}; the clear post has media field: ${'media' in clearPost}; the hub sees ${hubSees.join(', ')}\n`);

const gate = [
  ['a photo is listed by its hash alone, costs ~47 bytes in the head, and a pinned reader takes it as new rather than as an insertion',
    headAfter - headBefore < 55 && listed.verdict === 'ok' && listed.note.length === 0 && listed.media?.get(hP)?.equals(photo) && coldListed.verdict === 'ok'],
  ['a withheld photo and a swapped photo are both the host, and the hash is what catches the swap',
    withheld.verdict === 'host' && swapped.verdict === 'host' && swapped.why.includes('not what the head lists')],
  ['a post naming a photo the head does not list is an ordinary post with a photo that is not there — no accusation',
    unlisted.verdict === 'ok' && unlisted.posts?.has(4) && !unlisted.media?.has(stray)],
  ['a photo is withdrawn like a post, the rewrite drops its lines, and the bytes stay served — withdrawal is not deletion',
  ...
  ['a listed photo survives a rewrite listed by hash alone', twoRewrites.verdict === 'ok' && twoRewrites.media?.has(h2) && !twoRewrites.media?.has(hP)],
  ...
  ['a sealed post\'s photo is ciphertext at a listed hash, the reference is inside the envelope, and the hub learns the size',
    sealed.verdict === 'ok' && !('media' in clearPost) && hubSees.length === 1 && hubSees[0].endsWith('20000 B')],
];
