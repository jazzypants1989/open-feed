...
// Kill criteria: a sealed post the reader refuses; a thread the reader cannot assemble across
// origins; a hub that needs any header beyond If-Match; a hub operator who reads a sealed post;
...
const FLOOR = 1024;
function seal(inner, to) {                                   // `to`: [{ genesis, read }] — the audience goes inside
  const e = x25519(), ck = crypto.randomBytes(32);
  const text = JSON.stringify({ ...inner, to: to.map((r) => r.genesis) });
  const plain = Buffer.from(JSON.stringify({ ...inner, to: to.map((r) => r.genesis), pad: ' '.repeat(Math.max(0, FLOOR - text.length)) }));
  return { epk: e.x, slots: to.map((r) => box(kdf(e.privateKey, r.read), ck).toString('base64url')), ct: box(ck, plain).toString('base64url') };
}
function open(sealed, me) {                                  // trial-decrypt every slot: nothing in the clear names a recipient
  for (const s of sealed.slots) {
    try { const ck = unbox(kdf(me.privateKey, sealed.epk), Buffer.from(s, 'base64url')); return JSON.parse(unbox(ck, Buffer.from(sealed.ct, 'base64url')).toString()); } catch { /* not my slot */ }
  }
  return null;
}
...
const claim = (p, pseq = 1, extra = {}) => pub.profile({ genesis: p.key.x, pseq, chain: [{ key: p.key.x }], recovery: pub.commit(1, [recoverer]), locations: [p.at], read: p.read.x, ...extra }, p.key);
for (const p of family) await io.put(`${p.at}/profile`, claim(p), null);
// A profile with no head yet reads as "no head served" — this host is misbehaving — so a brand-new
// identity writes an empty head before anyone looks (a sentence the spec owes; see the card).
const fresh = await read(get, { learned: mom.key.x, at: mom.at });
for (const p of family) await pub.resignHead(io, p.at, p.key);
...
const readKeyOf = async (p, pinned) => {
  ...
  if (crypto.createHash('sha256').update(body).digest('base64url') !== pinned.phash) return null;
...
const audienceOf = async (ps) => Promise.all(ps.map(async (p) => ({ genesis: p.key.x, read: await readKeyOf(p, seen.get(p.key.x)) })));

// 1. Mom: a family-only post, sealed to Jesse and Cousin (and herself, so her other device reads it).
const fam = await audienceOf([mom, jesse, cousin]);
const n1 = await pub.publish(io, mom.at, mom.key, 1, { at: '2026-08-10T09:00:00Z', sealed: seal({ text: 'the scan came back clear' }, fam) });
...
const jn2 = await pub.publish(io, jesse.at, jesse.key, 2, { at: '2026-08-10T11:05:00Z', sealed: seal({ rel: 'reply', target: momTarget(1), text: 'and this one hides whom it answers' }, fam) });
// 3. Cousin: a reaction, sealed because the parent was.
const cn1 = await pub.publish(io, cousin.at, cousin.key, 1, { at: '2026-08-10T12:00:00Z', rel: 'like', target: momTarget(1), sealed: seal({}, fam) });
...
const opened = (who, me) => [...views.get(who).posts].map(([n, o]) => ({ n, rel: o.rel, target: o.target, ...(o.sealed ? open(o.sealed, me.read) ?? { locked: true } : {}) }));
...
const disk = (hub) => [...hub.files].map(([k, f]) => { const o = JSON.parse(f.subarray(0, f.lastIndexOf(0x0a)).toString()); return { file: k, ...(k.endsWith('profile') ? { genesis: o.genesis.slice(0, 8) + '…', locations: o.locations, read: o.read.slice(0, 8) + '…' } : k.endsWith('head') ? { lists: o.entries.map(([n]) => n), top: o.top } : { n: o.n, at: o.at, rel: o.rel ?? null, target: o.target ? `${o.target.key === mom.key.x ? 'mom' : '?'}#${o.target.n}` : null, sealed: o.sealed ? `${o.sealed.slots.length} slots, ${o.sealed.ct.length} B` : null, text: o.text ?? null }) }; });
const opensOnDisk = (hub) => [...hub.files.values()].map((f) => JSON.parse(f.subarray(0, f.lastIndexOf(0x0a)).toString())).filter((o) => o.sealed).map((o) => open(o.sealed, stranger.read)).filter(Boolean).length;
...
// The sealed-target reply (5): no rumor for a reader that is not a recipient; a recipient opens it and can raise it.
...
const inner5 = open(jesseFeed.posts.get(5).sealed, cousin.read);
const raisedSealedRecipient = await rumors(get, new Map([[mom.key.x, momAfter.pin]]), new Map([[5, inner5]]), 'jesse');

// 7. Mom leaves the ex's hub for a third one. She writes the same files there, a profile at pseq 2
...
await io.put(`${momNew}/profile`, claim(mom, 2, { locations: [momNew], prev: momAfter.pin.phash }), null);
await io.put(`${momNew}/head`, M.files.get('mom/head'), null);
...
const exRead = x25519();
const exProfile = pub.profile({ genesis: mom.key.x, pseq: 1, chain: [{ key: mom.key.x }], recovery: pub.commit(1, [recoverer]), locations: [mom.at], read: exRead.x }, pub.newKey());
M.swap.set('mom/profile', exProfile);
const naiveKey = await naiveReadKeyOf(mom), checkedKey = await readKeyOf(mom, momAfter.pin);
const naiveSealed = seal({ text: 'for mom only' }, [{ genesis: mom.key.x, read: naiveKey }]);
const exReads = open(naiveSealed, exRead);
M.swap.delete('mom/profile');

// ---- what was seen ----
console.log(`\n  a profile with no head yet reads: ${fresh.verdict} — ${fresh.why}`);
...
for (const [who, hub] of [['Mom\'s hub (the ex)', M], ['Jesse\'s hub', J]]) {
  ...
  console.log(`      sealed posts he can open with his own key: ${opensOnDisk(hub)}`);
}
...
console.log(`    the sealed-target reply (#98): a non-recipient raises [${raisedSealedOutsider}]; a recipient who opened it raises [${raisedSealedRecipient}]`);
console.log(`\n  Mom moves`);
console.log(`    Cousin's reader knew only the ex's hub; after Jesse's reply it holds pseq ${followed.pseq}, top ${followed.top}, and raised [${raisedAfterMove}]`);
...
const gate = [
  ['a sealed post is an ordinary post to the unchanged reader: all three identities read ok across two origins',
  ...
  ['a stranger holding the same bytes opens none of the sealed posts',
    Object.values(asStranger).flat().filter((p) => p.locked).length === 5],
  ['neither operator opens a sealed post from his own disk',
  ...
  ['a sealed target raises no rumor for anyone outside the audience, and does for a recipient who opened it',
  ...
  ['a claimed name with no head yet is "this host is misbehaving" — a publisher writes an empty head before anyone looks',
    fresh.verdict === 'host' && fresh.why === 'no head served'],
