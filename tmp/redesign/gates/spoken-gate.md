...
**Review gate** (2026-08-23 review, finding A7 and text defect C1). Substrate: the §4.1 derivation
verbatim (the one `tmp/regen2.js` uses for B.12) over keys from the weekend publisher.

**Question.** §4.1 derives the code from the *genesis* key and offers it "for the moment a person
is shown two versions of one identity". Two versions of one identity share the genesis. And §4.1
...
**Method.** Alice's branch and a thief's, same genesis, different current keys: the code over the
genesis, then over each branch's current key. Then a grind: keygen + HKDF per try for 1.5 s on
...
**Numbers** (stale if §4.1's derivation changes; the rate is this machine's, 30,872 tries/s):

- Same genesis → **the same six words** on both branches. Over the current keys they differ, and
...
**Kill criterion.** Two branches of one identity speaking different codes under §4.1 as written;
a rate making 40 bits sub-second. **Not triggered.**

**Revert-checked** (`revert.js`): deriving the repaired code from the genesis instead of the
current key makes row 2 collide.

**Verdict.** The code distinguishes *identities*, never two branches of one: §4.1's contest clause
is false as written. Repair: define the code over *a key*; let the out-of-band route carry the
current key as well as the genesis, and let a reader given one follow the branch containing it —
the only exit from `identity: contested` the text has. Replace the 40-bit sentence with the table.

**Run:** `node tmp/redesign/gates/spoken-gate.js`
