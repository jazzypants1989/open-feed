...
**Question.** §5.5 lists a scheduled post as `[n, hash, "pending"]`: a reader MUST NOT fetch it,
and it is confirmed by a bare line in a head only the device can sign. RULINGS §11.5 kept it with
...
same two reads against the same head with the pending line deleted. Friday the hub tries to
...
**Numbers** (stale if §5.5 or the reader's fetch step change):

- Between the line and the confirmation, pinned and cold: `ok [1–7, 9, 10] pending: 8`; post 8
  fetched **0** times. With the line deleted from the same head: **identical** but for the note.
- The hub appends the confirmation: pinned `ok` with *no head I can verify*, post 8 absent; cold
...
- Without the line, the scheduled post lands at **10** at release time, `ok` everywhere; the
  device has the key and the bytes and §6.1 signs the number in, so it needs nothing. Listing
...
- Cost: **18 spec lines**, 7 reader lines, 4 publisher lines — the entry table, the fold's
  confirmation clause, the rewrite's carry-through, §8.4's exemption, a note, a §14.2 row, a pin
  field, and B.10/B.11.
...
row red; removing `top: 8` from Monday's head makes the confirmation read as a backdate.
...
buys a stable number for scheduled posts at the price above, and §5.5 must then state that
purpose, since the text states only the cost. **The owner rules** (plan-mode answer 2026-08-23:
after this gate). Cutting it reverses RULINGS §11.5.

**Held to the ruling (2026-08-23, RULINGS §14.5 — cut).** The gate now asserts the ruled design: a
head carrying the retired three-element line does not fold; the scheduled post lands at the next
...
**Run:** `node tmp/redesign/gates/pending-gate.js`
