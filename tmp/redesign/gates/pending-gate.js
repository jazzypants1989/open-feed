// pending-gate: §5.5's pending line, measured before the ruling and held to after it. Before: a
// scheduled post was listed [n, hash, "pending"], never fetched, and confirmed by a bare line in a
// head only the device could sign — so nothing any reader saw before the device acted depended on
...
// holds the design to the ruling. Kill criteria: a head carrying the old line that folds; a
...
const profile = pub.profile({ genesis: G.x, pseq: 1, chain: [{ key: G.x }], recovery: REC, locations: ['https://alice.example'] }, G);
...
await pub.amendHead(a.net, AT, G, (h) => ({ ...h, entries: [...h.entries, [8, pub.address(p8), 'pending']], top: 8 }));
const pinnedOld = await a.see(a.pin), coldOld = await a.see(null);
claim(`a head carrying [8, hash, "pending"]: pinned ${view(pinnedOld)} · cold ${view(coldOld)}`, pinnedOld.verdict === 'host' && coldOld.verdict === 'host' && pinnedOld.why === 'the head does not fold');
...
claim('the device needs nothing it does not have — it holds the key and the bytes, and §6.1 puts the number inside the signature', doneB.posts.get(10).text === 'scheduled' && doneB.posts.get(10).n === 10);
...
await pub.amendHead(c.net, AT, G, (h) => ({ ...h, entries: [...h.entries, [8, pub.address(p8)]] }));
...
const spec = fs.readFileSync(path.join(here, '..', '..', '..', 'open-feed-spec-2.md'), 'utf8');
const specLines = spec.split('\n').filter((l) => /\bpending\b/i.test(l) && !/pending-gate/.test(l)).length;
const codeLines = ['weekend-reader.js', 'weekend-publisher.js'].map((f) => fs.readFileSync(path.join(here, f), 'utf8').split('\n').filter((l) => /\bpending\b/.test(l)).length);
