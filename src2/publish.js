// §9 (the client side) and §11 — a publisher: a device holding the key, writing signed files to any
...
import { signHead, liveEntries } from './head.js';
...
/**
 * `keep(path, bytes)` is §11: the app MUST keep the signed bytes of everything it publishes. The
...
export function createPublisher({ io, key, at, keep = null }) {
  ...
  /** §9.4: claim a name — the profile, then an empty head, so a brand-new identity never reads as a misbehaving host. */
  ...
  /** §9.2: take the next free number at or above `n`. */
  ...
  /** §9.1: re-read the head the hub is serving and fold the change into that; the tag is the hub's. */
  async function amendHead(change) {
    for (let attempt = 0; attempt < 5; attempt++) {
      const cur = await io.get(`${at}/head`);
      if (cur && !cur.etag) throw new PublishError('the hub sent no ETag', 0);
      let obj = { entries: [], hseq: 0, top: 0 };
      if (cur) { try { obj = parseBody(splitFile(cur.bytes).body); } catch { throw new PublishError('the served head does not parse', 0); } }
      const next = change({ entries: obj.entries, hseq: obj.hseq + 1, top: obj.top });
      const r = await putKept('/head', signHead(next, key), { ifMatch: cur?.etag ?? null });
      if (r.status === 200) return next;
      if (r.status !== 412) throw new PublishError(`head: ${r.status}`, r.status);
    }
    throw new PublishError('head: gave up after five races', 412);
  }
  /** §9.3: the post, then the head that lists it. */
  const publish = async (n, fields) => { const { n: num, entry } = await publishPost(n, fields); await amendHead((h) => ({ ...h, entries: [...h.entries, entry], top: Math.max(h.top, num) })); return num; };
  const publishMedia = async (bytes) => { const h = sha256(bytes); const r = await putKept(`/media/${h}`, bytes, { contentType: 'application/octet-stream' }); if (r.status !== 201 && r.status !== 200) throw new PublishError(`media: ${r.status}`, r.status); await amendHead((hd) => ({ ...hd, entries: [...hd.entries, [h]] })); return h; };
  const withdraw = (n) => amendHead((h) => ({ ...h, entries: [...h.entries, [n, null]] }));
  const relist = (n, hash) => amendHead((h) => ({ ...h, entries: [...h.entries, [n, hash]] }));
  const resignHead = () => amendHead((h) => h);
  const rewrite = () => amendHead((h) => ({ ...h, entries: liveEntries(h.entries) }));
  /** §12: the views are unsigned overwritable files at conventional paths. */
  ...
  return { claim, updateProfile, publishPost, amendHead, publish, publishMedia, withdraw, relist, resignHead, rewrite, putView, copy };
}
