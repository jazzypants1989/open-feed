// §7 — sealed content. One X25519 ephemeral per message; per recipient a blinded tag, a wrapped
// content key; the content under a single-use key with the carrier bound as associated data; the
// audience inside, naming people; padding to a bucket with random dummies. And §5.4's sealed photo.
...
// §7.2: the content's associated data is the ephemeral key and the carrier — the author's genesis
...
export const carrierOf = (genesis, n) => `${genesis}:${n}`;
export const bucket = (n, floor) => Math.max(floor, 1 << Math.ceil(Math.log2(Math.max(n, 1))));
export const FLOOR = { slots: 8, body: 512 };

/**
 * Seal `content` to `audience` — entries `{ key, read, loc }` (§7.5) — for the post at `carrier`.
 * `policy` is `'floor'` (§7.4's SHOULD) or `'pow2'`; `random`, `ephemeral`, `contentKey` are seams
 * for reproducible vectors and nothing else.
 */
export function seal({ content, audience, carrier, policy = 'floor', random = crypto.randomBytes, ephemeral, contentKey }) {
  if (typeof carrier !== 'string' || !carrier) throw new TypeError('a carrier is required (§7.2)');
  if (!Array.isArray(audience) || !audience.every((a) => a && typeof a.key === 'string' && typeof a.read === 'string' && typeof a.loc === 'string')) throw new TypeError('audience entries are {key, read, loc} (§7.5)');
  ...
  if (plain.length > MAX_PLAIN) throw new RangeError(`sealed plaintext is limited to ${MAX_PLAIN} bytes (§7.1)`);
...
export function open(env, privateKey, carrier) {
...
export function sealPhoto(photo, random = crypto.randomBytes) {
  const key = random(32);
  const bytes = aead(key, ZERO12, photo, Buffer.alloc(0));
  return { bytes, hash: crypto.createHash('sha256').update(bytes).digest('base64url'), key: b64(key) };
}
export function openPhoto(bytes, key) { try { return unaead(unb64(key), ZERO12, bytes, Buffer.alloc(0)); } catch { return null; } }
