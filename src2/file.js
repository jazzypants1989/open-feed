// §3 — the file format. A signed file is its body bytes, one `\n`, then an Ed25519 signature over
...
// §3.1: an 86-character base64url line that re-encodes to itself. Both halves are needed — base64
...
/** §3.1's body rules plus §3.4's parser: a UTF-8 JSON object, no BOM, no `\n`. */
...
export function signFile(obj, key) {
  ...
  parseBody(body);                                                   // §3.4's producer half: never emit what a reader rejects
...
/** The address of a file is the hash of its body — never of the whole file (§3.2). */
