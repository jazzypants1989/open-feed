// §5 — the head: the signed list saying what exists now. The live set is a fold over the entries;
// a number has one hash, ever; a photo is listed by its hash alone; `top` never decreases.
import { signFile, verifyFile } from './file.js';

/** §5.2: the fold. Returns `{ live: Map, top }` or null when the head is invalid. */
export function fold(entries) {
  ...
  for (const e of entries) {
    ...
    const [a, b] = e;
    if (typeof a === 'string') {                                          // a photo: [hash] or [hash, null]
...
/** A head's shape: entries first (§5), a non-negative `hseq`, a `top` at or above the highest number. */
export function checkHead(obj, set) {
  if (Object.keys(obj)[0] !== 'entries') return 'entries is not the first member';
  if (!Number.isInteger(obj.hseq) || obj.hseq < 0) return 'hseq is not a non-negative integer';
  if (!Number.isInteger(obj.top) || obj.top < set.top) return 'top is below the highest number issued';
  return null;
}

/**
 * §8.2 step 9: a served head against the pin. Returns `{ verdict, why }` on a refusal, else
 * `{ notes, withdrawn }` where `withdrawn` is the map of withdrawn numbers to the hash they had.
 */
export function checkAgainstPin(head, set, pin) {
  const bad = (why) => ({ verdict: 'host', why });
  if (head.obj.hseq < pin.hseq) return bad('a head older than the one this reader saw');
  if (head.obj.hseq === pin.hseq && head.address !== pin.hhash) return bad('two heads at one version');
  if (head.obj.top < pin.top) return bad('the highest number used went backwards');
...
/** The publisher's side: the entries a rewrite keeps (§5.7) — the live set, in order. */
...
export const signHead = ({ entries, hseq, top }, key) => signFile({ entries, hseq, top }, key);
export const verifyHead = (bytes, currentKey) => verifyFile(bytes, currentKey);
