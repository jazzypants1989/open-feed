// §9 — the outbound fetch layer. Nothing else in `src/` opens a socket.
//
...
// 2. Redirects are followed by hand, because §9's rule — never to a different origin, each link
...
export function createFetcher({
  ...
  async function request(rawUrl, { method = 'GET', headers = {}, body, followRedirects = method === 'GET', max } = {}) {
    ...
    let parsed = startUrl, links = 0, current = null, timedOut = false;
    ...
    try {
      for (;;) {
        ...
        if (followRedirects && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          req.destroy();
          if (++links > maxRedirects) throw new FetchError(`more than ${maxRedirects} redirects from ${startUrl.href}`, { code: 'too_many_redirects', url: startUrl.href });
          const next = assertUrlAllowed(new URL(res.headers.location, parsed).href, { from: parsed.href });
          if (next.origin !== parsed.origin) throw new FetchError(`refusing cross-origin redirect ${parsed.origin} -> ${next.origin}: moving is expressed in the profile, not a Location header (§9)`, { code: 'cross_origin_redirect', url: startUrl.href });
