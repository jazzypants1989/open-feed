...
**Spec:** §3.1, and the `anchor` rule in §3. §3.4 is the contest the last block points at.
...
posts — hangs off it, and all of it arrives through a hub. So the first question a reader has to
answer is the one question the hub cannot be allowed to answer: **which key is this?**

A reader that learns your key from the hub it is reading has learned nothing, because the hub
chose both the profile and the key printed in it. §3.7 gives two routes that go around the hub — a
...
**A reader that learns your key from the hub has learned nothing.** The example has a hostile hub
serve a profile that is well formed in every respect: valid signed file (§2.1), chain that walks
(§3.3), a recovery list, a `name` of "Alice". It is simply a profile for a key the hub generated.
Verified against the key printed inside it — trust on first use — it comes back `ok`. Verified
against the key the reader learned out of band, `verifyProfile` returns `contested: not the identity
...
choose it for you, but §3.1 forbids using it to resolve or match an identity.
...
**The spoken code.** Six words for a phone call, derived exactly as §3.7 writes it:
...
call — and the code confirms which key that candidate must have. The hub supplies the bytes; the
person supplies the answer to *which*.

**66 bits, and why not 55.** That confirmation is only as good as the odds against a hub finding
...
36,028,797,018,963,968 — 2,048 times less work, and §3.7 puts the difference plainly: at 66 bits
...
**The exit from contested.** That limit is why §3.4 needs a way out. When two profiles claim one
identity and no majority of the recovery list settles it, the reader stops at **contested** and
follows neither branch. §3.7's answer is that the same two routes MAY carry the key the owner's
...
key's. It is one block here on purpose; `examples/contests/` is where §3.4 lives.
...
**Spec:** §3.2 the chain, and §4.4 for what changing a key costs in practice.
**Run:** `node examples/identity/identity.js`
...
A link with `signature` is a **rotation** — the previous key signed the move, so the owner made it. A link
...
message, and nothing here that a hub issues or countersigns.
...
carried copy (§3.4 rule 3) — the carried copy is for the cold reader only. The list itself is the
next block; what happens when two chains disagree about one identity is `examples/contests/`.
...
`signature`, two vouchers that now count. Nothing after that link is disturbed. That matters because
§3.4 settles a contest by a **majority of the recovery list at the divergence point**, never by a `signature` — so a
...
**A restore changes the key and nothing else.** A link with no `signature` MUST NOT arrive, in the same
profile version, with a change to `locations`, `recovery`, `name` or `read`, and a checkpointed reader
MUST catch it. The example checkpoints a reader at version 2 and then serves it version 3 twice: once
honestly, which reads `ok`, and once with `locations` moved to another hub, which reads
...
**§3.3 — changing the key means writing the profile and the index again, in that order.** The last
...
the profile. That is not a leak the design tolerates, it is a requirement: §3.4 counts a majority
against the list, and a majority needs a denominator every reader can see. `examples/contests/` is
...
settles a contested identity, and `examples/contests/` shows why one bar is the point.
