// §4 — the index: entries and the fold, `top`, media, who signs it, rewriting.
...
const idx = (entries, version, top, k = A3) => signIndex({ entries, version, top }, k);
...
assert.equal(checkIndex({ version: 1, top: 3, entries: [] }, fold([])), 'entries is not the first member');
assert.equal(checkIndex({ entries: [], version: -1, top: 0 }, fold([])), 'version is not a non-negative integer');
assert.equal(checkAgainstPin({ obj: { version: 0, top: 3 }, address: 'x' }, fold([]), cold.pin).why, 'an index older than the one this reader saw');
...
assert.deepEqual([[...fold(e2).live.keys()], fold(e2).top, fold([...e2, [hp, null]]).live.has(hp)], [[1, 3, 4, hp], 4, false]);
...
console.log('§4.2 — top\n');
console.log(`  post 3 withdrawn: top ${fold([[1, h1], [2, h2], [3, h3], [3, null]]).top}; an index with top 2 after it: ${dropped.verdict}: ${dropped.why}\n`);
assert.equal(fold([[1, h1], [2, h2], [3, h3], [3, null]]).top, 3);
assert.deepEqual([dropped.verdict, dropped.why], ['host', 'the highest number used went backwards']);
rule('4.2', '`top` MUST NOT decrease, even when the post holding that number is withdrawn.');
...
const env = encrypt({ content: { text: 'the morning after', rel: 'root', media: [{ hash: sealed.hash, key: sealed.key }] }, audience: [{ key: mum.x, read: xkey('mum-read').x, loc: 'https://mom.example/mom' }],
...
assert.deepEqual(Object.keys(parseBody(splitFile(p7).body)), ['n', 'at', 'encrypted']);
...
assert.deepEqual([[...fold(e4).live.keys()], fold(e4).top], [[...fold(kept).live.keys()], fold(kept).top]);
