// §4 — the index: entries and replay, `highest`, media, who signs it, rewriting.
...
import { replay, checkIndex, checkAgainstCheckpoint, liveEntries, signIndex, verifyIndex } from '../../src/index.js';
import { createReader } from '../../src/reader.js';
import { encrypt, decrypt, postBinding, readingKeyFromSeed, encryptMedia, decryptMedia } from '../../src/envelope.js';
...
const shown = (s) => (s === null ? 'entries are invalid' : [...s.live.keys()].map((k) => (typeof k === 'string' ? 'media' : k)).join(' ') || 'nothing live');
...
const serve = async (index, checkpoint = null, now) => { files.set(`${AT}/index`, index); return reader.read({ learned: A1.x, at: AT, checkpoint, now }); };
...
assert.equal(checkIndex({ version: 1, highest: 3, entries: [] }, replay([])), 'entries is not the first member');
assert.equal(checkIndex({ entries: [], version: -1, highest: 0 }, replay([])), 'version is not a non-negative integer');
assert.equal(checkAgainstCheckpoint({ obj: { version: 0, highest: 3 }, address: 'x' }, replay([]), cold.checkpoint).why, 'an index older than the one this reader saw');
...
console.log('§4.1 — replay\n');
for (let i = 1; i <= e2.length; i++) console.log(`  ${JSON.stringify(e2.slice(0, i).at(-1)).replace(/"[\w-]{43}"/, '"…"').padEnd(14)} → ${shown(replay(e2.slice(0, i)))}`);
assert.deepEqual([[...replay(e2).live.keys()], replay(e2).highest, replay([...e2, [hp, null]]).live.has(hp)], [[1, 3, 4, hp], 4, false]);
for (const [what, e] of [['a number listed twice', [[1, h1], [2, h2], [2, h3]]], ['a withdrawal of nothing', [[1, h1], [9, null]]], ['a number below 1', [[0, h1]]],
  ['a media file listed twice', [[hp], [hp]]], ['a media withdrawal of nothing', [[1, h1], [hp, null]]], ['re-listed at another hash', [[1, h1], [2, h2], [2, null], [2, hAlt]]]]) assert.equal(replay(e), null, what);
assert.equal(replay([[1, h1], [2, h2], [2, null], [2, h2]]).live.get(2).hash, h2);           // re-listed at the identical hash
...
assert.deepEqual([wont.verdict, wont.why], ['tampered', 'the index entries are invalid']);
// Across indexes, the checkpoint remembers the withdrawn hash.
const gone = await serve(idx([[1, h1], [2, h2], [3, h3], [2, null]], 2, 3), cold.checkpoint);
const back = await serve(idx([[1, h1], [3, h3], [2, h2]], 3, 3), gone.checkpoint);
const swap = await serve(idx([[1, h1], [3, h3], [2, hAlt]], 3, 3), gone.checkpoint);
console.log(`\n  withdrawn, then back at the same hash   ${back.verdict}`);
console.log(`  withdrawn, then back at another hash    ${swap.verdict}: ${swap.why}\n`);
assert.deepEqual([gone.verdict, gone.checkpoint.withdrawn.get(2), back.verdict, swap.verdict, swap.why], ['ok', h2, 'ok', 'tampered', 'post 2 changed after the reader saw it']);
rule('4.1', `| line | means |
|---|---|
...
// ---- §4.2 highest ----
const dropped = await serve(idx([[1, h1], [2, h2]], 4, 2), gone.checkpoint);
console.log('§4.2 — highest\n');
console.log(`  post 3 withdrawn: highest ${replay([[1, h1], [2, h2], [3, h3], [3, null]]).highest}; an index with highest 2 after it: ${dropped.verdict}: ${dropped.why}\n`);
assert.equal(replay([[1, h1], [2, h2], [3, h3], [3, null]]).highest, 3);
assert.deepEqual([dropped.verdict, dropped.why], ['tampered', 'the highest number used went backwards']);
...
assert.deepEqual([swapped.verdict, swapped.why], ['tampered', `media file ${hp} is not what the index lists`]);
...
// ---- §4.4 who signs ----
const thief = idx([[1, h1]], 9, 3, A2), nopin = await serve(thief), held = await serve(thief, cold.checkpoint);
console.log('§4.4 — who signs\n');
console.log(`  an index signed by the rotated-away key   no checkpoint: ${nopin.verdict}: ${nopin.why};   checkpointed: ${held.verdict}, ${held.note.join('; ')}\n`);
assert.deepEqual([verifyIndex(thief, A3.x), verifyIndex(v1, A3.x) !== null, nopin.verdict, nopin.why, held.verdict], [null, true, 'tampered', 'the index is not signed by the key the profile ends on', 'ok']);
...
const at6 = await serve(idx(kept, 6, 4), at1.checkpoint, T1);
...
assert.deepEqual([[...replay(e4).live.keys()], replay(e4).highest], [[...replay(kept).live.keys()], replay(kept).highest]);
assert.deepEqual([at6.verdict, at6.note, [...at6.posts.keys()].sort()], ['ok', ['withdrawn: 2', 'withdrawn: 3'], [1, 4]]);
rule('4.5', `A publisher MAY replace the index with replay of its entries, at a higher \`version\`. A reader accepts a
