...
const who = (p) => ({ key: p.ed.x, read: p.read.x, loc: p.loc });
...
assert.deepEqual(Object.keys(JSON.parse(splitFile(post5).body)), ['n', 'at', 'encrypted']);
...
assert.equal(decrypt({ ...dm, epk: zeroPub }, mum.read.privateKey, c5), null);
assert.throws(() => encrypt({ content: { text: 'x' }, audience: [{ key: mum.ed.x, read: zeroPub, loc: mum.loc }], carrier: c5, ephemeral: eph, contentKey: key5 }));
// The content key and the ephemeral are per message.
assert.notEqual(dm.epk, fam.epk);
...
for (const a of inside.audience) assert.deepEqual(Object.keys(a), ['key', 'read', 'loc']);
...
const reply = encrypt({ content: { text: 'we are with you', rel: 'reply', target: { key: alice.ed.x, n: 6, hash: address(post6), loc: AT }, media: [{ hash: 'h'.repeat(43), key: 'k'.repeat(43) }] }, audience: inside.audience, carrier: c13, ephemeral: xk('example:ephemeral/13'), contentKey: ck('example:contentkey/13') });
const b13 = splitFile(signFile({ n: 13, at: '2026-08-19T19:00:00Z', encrypted: reply }, mum.ed)).body.toString();
const opened = decrypt(reply, alice.read.privateKey, c13);
console.log(`§6.5 — mum's reply: "reply" in the public bytes? ${b13.includes('reply')}; inside: rel ${opened.rel}, target n ${opened.target.n}, media ${JSON.stringify(Object.keys(opened.media[0]))}\n`);
assert.ok(!b13.includes('reply') && !b13.includes(alice.ed.x));
assert.deepEqual([opened.rel, Object.keys(opened.target), opened.target.hash, Object.keys(opened.media[0])], ['reply', ['key', 'n', 'hash', 'loc'], address(post6), ['hash', 'key']]);
