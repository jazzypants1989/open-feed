...
- **Step 4 before step 5.** Her rotated profile does not verify under the anchor key and does verify
  under the key the chain ends on. Which key to verify under is the chain's last word (§3.3, §4.4).
...
§7.4 — step 11, targets and the rumor rule — is the one place a read reaches past the identity it
...
**Spec:** §4.3 for `top`, §7.4 for targets and the rumor rule, §7.3 for why a rumor is a note and
not a verdict.
...
withdrew it, the live set is `1, 2`, and `top` is 3. Then she rewrites (§4.5) and both lines about 3
