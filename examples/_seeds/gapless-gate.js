...
// (ruling 3), "above the top" (§10.1), the reclaim rule (§11.5) — and nothing had stressed it. Over
// a real socket, with the UNCHANGED weekend reader and publisher: a device that crashes between the
// post write and the index write; two devices and a crash; a griefer holding numbers the owner then
// reclaims; an abandoned draft; and the pending entry's lifecycle across a rewrite and across the
// fallback a rotation forces — the two paths HANDOFF-final-review.md §2 said nothing checks.
...
// ---- the hub, as §11.5 rules it: checks nothing on the ordinary path, resolves a collision ----
...
const bobReplies = async (s, targets) => {
  await s.net.put(`${BAT}/profile`, pub.profile({ anchor: B.x, version: 1, chain: [{ key: B.x }], recovery: REC, locations: ['https://bob.example'] }, B), null);
  let n = 1;
...
// 1. the crash: the post write lands, the index write never happens, and the device comes back with
...
const orphanWithdrawn = await s1.net.put(`${AT}/index`, pub.index({ ...JSON.parse(body(await s1.net.get(`${AT}/index`))), entries: [...JSON.parse(body(await s1.net.get(`${AT}/index`))).entries, [8, null]], version: 99 }, A), s1.hub.tag('alice/index'));
const orphanWithdrawnRead = await at(s1);
await s1.net.put(`${AT}/index`, pub.index({ ...JSON.parse(body(await s1.net.get(`${AT}/index`))), entries: JSON.parse(body(await s1.net.get(`${AT}/index`))).entries.slice(0, -1), version: 100 }, A), s1.hub.tag('alice/index'));
...
await pub.amendIndex(s2.net, AT, A, (h) => ({ ...h, entries: [...h.entries, lost.entry] }));
const lateListed = await read(s2.net.get, { learned: A.x, at: AT, pin: beforeLate.pin });
const lateCold = await cold(s2);
// ...and the attack the same reader rule exists for: the custodian of §12.2, holding her current key
...
await pub.amendIndex(s3.net, AT, A, (h) => ({ ...h, entries: [...h.entries, [3, pub.address(backdated)]] }));
...
// 5. scheduled posts (the `pending` line was cut 2026-08-23 — pending-gate.md). A index carrying the
...
await pub.amendIndex(s6.net, AT, A, (h) => ({ ...h, entries: [...h.entries, [8, pub.address(draft), 'pending']], top: 8 }));
const oldLine = await at(s6);
await pub.amendIndex(s6.net, AT, A, (h) => ({ ...h, entries: h.entries.filter((e) => e.length < 3) }));
...
await pub.amendIndex(s7.net, AT, A, (h) => ({ ...h, entries: [...h.entries, [8, pub.address(draft)]] }));
const late = await at(s7);

// 6. the write order: the index that lists a post must come after the post's bytes, or a reader in
// between accuses an honest host.
const s8 = await fresh();
const early = pub.post(8, { at: '2026-08-02', text: 'listed first' }, A);
await pub.amendIndex(s8.net, AT, A, (h) => ({ ...h, entries: [...h.entries, [8, pub.address(early)]], top: 8 }));
...
console.log(`    crash after post 8, before the index; restart forgetting:   next post lands at ${afterCrash}; pinned ${crashPinned.verdict} [${crashPinned.note}]; cold ${crashCold.verdict} [${crashCold.note}]`);
console.log(`      the orphan: served ${orphanServed ? 'yes' : 'no'}; her retake of 8: ${orphanRetaken}; a index withdrawing it: ${orphanWithdrawn} and reads ${orphanWithdrawnRead.verdict} (${orphanWithdrawnRead.why ?? 'ok'}); bob's reply to it raises: ${replyToOrphan.length ? replyToOrphan : 'nothing'}`);
...
console.log(`    index written before the post: ${headFirst.verdict} (${headFirst.why ?? 'ok'}); then the post lands: ${thenPost.verdict}\n`);
...
const gate = [
  ...
  ['a index carrying the retired pending line does not fold', oldLine.verdict === 'host' && oldLine.why === 'the index does not fold'],
  ...
  ['a index that lists a post before its bytes land accuses an honest host — the post is written first',
