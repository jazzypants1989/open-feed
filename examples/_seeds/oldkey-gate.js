// oldkey-gate: what a key she rotated away from can still do. §3.3 says a stolen old key is closed
// "without any revocation mechanism" because the index is signed by the current key. Two places
// the text does not reach: §8.5's reclaim rule calls a file "the owner's" when ANY chain key
// signed it, so an old key can hold every number she has not reached yet and she cannot take them
// back; and §4.2 forbids re-listing a withdrawn number at any hash, so what a thief withdrew while
// he held the current key stays withdrawn after her restore. Finding A2 and A5 of the 2026-08-23
// review. Kill criteria: the squat refused under §8.5 as written; a re-listing rule that lets a
...
async function squat(reclaim) {
  ...
  await pub.amendIndex(net, AT, K2, (h) => ({ ...h, entries: [...h.entries, [10, pub.address(oldListed)]], top: 10 }));
...
console.log('\nA5. The thief held K2, withdrew everything, rewrote the index. Alice restores to K3.\n');
const relistScene = async (thiefRewrites) => {
  ...
  await pub.rewrite(net, AT, K3);                                  // the lines for 2 are gone from this index
  await pub.amendIndex(net, AT, K3, (h) => ({ ...h, entries: [...h.entries, [2, pub.address(other)]] }));
...
for (const rewrites of [true, false]) {
  ...
  claim(`${rewrites ? 'after his rewrite' : 'with his withdrawal lines still in the index'}: a reader pinned before ${r.rs[0]} · pinned during ${r.rs[1]} · cold ${r.rs[2]} — all three hold the three posts`, r.rs.every((v) => v === 'ok') && r.posts.every((n) => n === 3));
...
// Within one index, the fold itself refuses a second hash for a number, for every reader.
...
await pub.amendIndex(net3, AT, K2, (h) => ({ ...h, entries: [...h.entries, [1, 'another-hash']] }));
const twoHashes = await read(net3.get, { learned: G.x, at: AT });
hub3.close();
claim(`within one index, a number re-listed at another hash: cold reader ${twoHashes.verdict}: ${twoHashes.why}`, twoHashes.why === 'the index does not fold');
claim('a host cannot do any of this: the index is signed, and only the current key signs it', (() => { const f = pub.index({ entries: [[1, 'x']], version: 99, top: 1 }, K2); const i = f.lastIndexOf(0x0a); try { return !crypto.verify(null, f.subarray(0, i), crypto.createPublicKey({ key: { kty: 'OKP', crv: 'Ed25519', x: K3.x }, format: 'jwk' }), Buffer.from(f.subarray(i + 1).toString(), 'base64url')); } catch { return false; } })());
