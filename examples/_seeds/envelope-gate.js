// envelope-gate: HANDOFF-final-review.md §1.A — the envelope, commissioned three times and never run.
// Today's src/enc.js construction (JWE: ECDH-ES+A256KW per slot, A256GCM, Concat KDF, a blinded
// 8-byte slot tag, the audience encrypted inside, carrier binding checked at the decrypting client)
...
// Then the half nobody has checked on this substrate: a encrypted post read by the UNCHANGED
// weekend-reader.js over a socket, and the carrier-binding attack §14.2.1 existed for, staged with
...
import { seal as oldSeal } from '../../archive/src/enc.js';
...
// owner ruled (the spec §6.2 MUST, §6.4 SHOULD), and one implementation is what keeps tools/regen.js's
// vectors from drifting away from the gate that proved them.
import { encrypt as sealNew, decrypt as openNew, bucket, xKey, xPub, b64, unb64, INFO } from './envelope.js';
...
const vecOut = vectors.map((v) => {
  // Dummies are random bytes (§6.4); a seeded stream stands in so the vector reproduces.
...
// Alice's post 5 is encrypted two ways: bound to its carrier (her anchor key and the number), and not.
const carrierOf = (anchor, n) => `${anchor}:${n}`;
...
await pub.publish(netA, '/alice', alice.ed, 5, { at: '2026-08-01T09:00:00Z', encrypted: bound });
await pub.publish(netA, '/alice', alice.ed, 6, { at: '2026-08-01T09:01:00Z', encrypted: unbound });
const aliceRead = await read(netA.get, { learned: alice.ed.x, at: '/alice' });
const sealedOpaque = aliceRead.verdict === 'ok' && aliceRead.posts.size === 2 && typeof aliceRead.posts.get(5).encrypted?.ct === 'string';
const mumAfter = openNew(aliceRead.posts.get(5).encrypted, fam[0].privateKey, carrierOf(alice.ed.x, 5))?.text;
const hostAfter = openNew(aliceRead.posts.get(5).encrypted, host.privateKey, carrierOf(alice.ed.x, 5));
// Signing did not change: the encrypted post's file is the same bytes-then-signature shape as a cleartext one.
...
// by his key, listed in his index, served from his hub. Mum's reader accepts his posts — they ARE his.
await pub.publish(netT, '/thief', thief.ed, 1, { at: '2026-08-02T09:00:00Z', text: 'she told me this herself:', encrypted: aliceRead.posts.get(5).encrypted });
await pub.publish(netT, '/thief', thief.ed, 2, { at: '2026-08-02T09:00:00Z', text: 'she told me this herself:', encrypted: aliceRead.posts.get(6).encrypted });
...
const liftedBound = openNew(thiefRead.posts.get(1).encrypted, fam[0].privateKey, carrierOf(thief.ed.x, 1));
const liftedUnbound = openNew(thiefRead.posts.get(2).encrypted, fam[0].privateKey, '');   // an unbound client passes nothing
...
const forgetful = openNew(thiefRead.posts.get(1).encrypted, fam[0].privateKey, '');
...
console.log(`  over the socket, the unchanged reader: alice ${aliceRead.verdict}, encrypted field opaque ${sealedOpaque}; mum opens → "${mumAfter}"; the host opens → ${hostAfter}`);
...
const gate = [
  ...
  ['the unchanged reader returns a encrypted post as ok with the field opaque — no new signing construction', sealedOpaque && sameShape],
  ...
  ['with no binding, the lifted envelope opens and her words render under his name — the attack §14.2.1 was for is alive on this substrate', liftedUnbound?.text === 'I am leaving him on Friday'],
];
