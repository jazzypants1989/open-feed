// weekend-gate: HANDOFF-to-spec.md §1.H, the minimality measure that has never been taken on this
...
const p2 = pub.profile({ anchor: A.x, version: 2, prev: pub.address(p1), chain: [...chain0, pub.rotation(A, A2, REC)], recovery: REC, locations: LOC }, A2);
...
const rolled = pub.index({ entries: [[1, good.live.get(1)]], version: 1, top: 1 }, A3);
hub.swap.set('alice/index', rolled);
const rollback = await read(net.get, { learned: A.x, at: AT, pin: good });
hub.swap.delete('alice/index');
...
const ex = pub.newKey();
const fake = pub.profile({ anchor: ex.x, version: 9, chain: [{ key: ex.x }], recovery: REC, locations: LOC }, ex);
...
const forked = pub.profile({ anchor: A.x, version: 3, prev: pub.address(p2), chain: [...chain0, pub.rotation(A, A2, REC), pub.restore(A2, ex, [sis], REC)], recovery: REC, locations: LOC }, ex);
...
const readerLines = sloc('../weekend-reader/weekend-reader.js'), pubLines = sloc('../weekend-publisher/weekend-publisher.js');
...
row('host serves an older index', rollback);
...
const gate = [
  ...
  ['a rotation is followed from the anchor key the reader learned, with no new trust',
    afterRotate.verdict === 'ok' && afterRotate.chain.current !== afterRotate.chain.keys[0]],
  ['between the two writes a rotation takes, an honest host is not accused — the reader keeps the index it verified',
    midRotation.verdict === 'ok' && midRotation.note.includes('no index I can verify') && midRotation.posts.size === 2],
  ...
  ['an older index, a swapped post, and a post that is not what the index lists are all the host',
  ...
  ['a post signed by a key that was hers, that the index does not list, is simply not there',
