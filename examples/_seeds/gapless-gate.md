...
**Candidate gate** (`../HANDOFF-final-review.md` §1.C and §2's two `pending` gaps). Substrate: the
weekend reader and publisher, unchanged in construction, over a real loopback socket, against the
hub §11.5 rules — checks nothing on the ordinary path, resolves a collision.

**Question.** Numbering is load-bearing three times — create-once (ruling 3), "above the top"
(§10.1), the reclaim rule (§11.5) — and nothing had stressed it. What does a crash between the post
write and the index write do? Does a gap read as a withdrawal of a post that never existed? Does
...
**Method.** Thirteen claims on fresh hubs. A device writes post 8 and dies before the index, then
...
abandoned, and released below the top after another device published; and the index written before
the post's bytes.
...
- The orphan is **served forever**: her own file holds 8, so her retake is 409 and a index
...
- Index before post: `host — post 8 is listed and not served`; once the bytes land, `ok`.
...
1. **The publisher needed the fold.** `rewrite` took the live list from the *reader's* pin, which
   ...
   not sent: `host`. Fixed: the publisher folds the index it is rewriting and keeps the flag (two
...
3. **"Numbering stays gapless" (SKETCH §5) is false under a crash, and it does not matter.** The
...
spec says a host MAY remove a numbered file the index does not list (with a grace window for the
post-before-index write order, claim 13), or it says plainly that withdrawal is not deletion.
...
abandons a number it cannot prove it listed. One sentence SKETCH §5 should lose ("gapless"), and
one question — is withdrawal ever deletion on an honest host — for the owner.

**Run:** `node examples/_seeds/gapless-gate.js`

**Re-measured 2026-08-23** after `pending` was cut (RULINGS §13.5; `pending-gate`): section 5 now
stages a index carrying the retired line (`host: the index does not fold`), a scheduled post landing
