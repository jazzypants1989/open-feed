// recovery-gate: the contest rule inside the composed reader. RULINGS §10.3 settles a fork by a
...
// said "contested" and stopped — and, as written, it contested only two profiles at ONE version, so a
// thief holding a rotated-out key who simply picked a higher number was followed by every reader.
// This gate puts the recovery into the reader, over a socket, and asks what it must have kept.
...
class Hub {
  ...
  handle(method, url, b, ifMatch) {
    ...
    if (m[2] === 'index' || m[2] === 'profile') { if (this.tag(key) !== ifMatch) return { status: 412 }; this.files.set(key, b); return { status: 200 }; }
...
const c0 = [{ key: A.x }], c2of = (rec) => [...c0, pub.rotation(A, A2, rec)];   // every link carries the list that stood before it
const prof = (version, chain, recovery, key) => pub.profile({ anchor: A.x, version, chain, recovery, locations: LOC }, key);
const hubs = [];
...
async function scene(recovery) {
  ...
  const serve = async (p, key) => { hub.files.set('alice/profile', p); await pub.resignIndex(net, AT, key); };
...
// 5. the thief edits the list FIRST — same chain, his own list, a higher version — and then forks with a
//    restore that satisfies the list he wrote. The recovery is the first list the reader saw there.
...
// 6. the thief forgets her restore: a higher version with the SHORTER chain, still ending on the key he holds.
...
// 7. what a cold reader has: the recovery it was handed. Cold on Alice's branch, her restore link carries
...
const readerLines = sloc('../weekend-reader/weekend-reader.js');
console.log(`\n  the reader is ${readerLines} lines with the recovery in it (141 before this review)\n`);
const show = (what, r) => console.log(`    ${what.padEnd(58)} thief first: ${r.thiefFirst.join(' → ').padEnd(60)} Alice first: ${r.aliceFirst.join(' → ')}`);
show('1. thief picks version 4, plain rotation; Alice 2 of 3', higherPseq);
...
console.log(`    6. thief serves version 9 with the chain from before her restore: ${who(forgotten)}`);
console.log(`    7. cold on Alice, then the thief: ${who(coldAliceThenThief)};  cold on the thief (forged recovery of one), then Alice: ${who(coldThiefThenAlice)}`);
...
const gate = [
  ['a thief holding a rotated-out key who picks a higher version is not followed by a reader that pinned the pre-fork profile — in either order',
    higherPseq.thiefFirst[1] === 'follows Alice' && higherPseq.aliceFirst[1].startsWith('host')],
  ['but a cold reader follows whichever branch it is served — the stated limit of §10.3',
    coldThief.verdict === 'ok' && coldThief.chain.current === T.x],
  ['a listed adversary who vouches for himself alone is refused outright by a reader holding the real recovery — his link is not even valid under it — in either order',
  ...
  ['rewriting the list before forking buys the thief nothing: the recovery is the first list the reader saw at that length',
  ...
  ['a cold reader\'s recovery is whatever its first profile carried: handed Alice\'s it later rejects the thief, handed the thief\'s it later rejects Alice',
    coldAliceThenThief.verdict === 'identity' && coldThiefThenAlice.verdict === 'identity'],
  ['an author who edits her list after a restore does not break her own chain — the link carries the list it satisfied',
  ...
  ['the reader with the recovery in it is still one file under 200 lines', readerLines < 200],
];
...
console.log('recovery-gate: all claims hold');
