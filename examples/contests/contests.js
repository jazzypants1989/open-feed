...
const read = (o, signer, checkpoint = null) => verifyProfile(signProfile(o, signer), { learned: A.x, checkpoint });
...
// ---- §3.4 rule 1: divergence ----
const alice = prof(3, [anchor, L1, restA3], family), checkpointed = checkpointTo(alice, A3);
const branch = prof(4, [anchor, L1, exRot], family), forgotten = prof(9, [anchor, L1], family);
console.log('§3.4 — the checkpoint holds the chain\n');
console.log(`  checkpointed      version 3   anchor → A2 → A3 (restored by mum and sis)`);
...
const rw = read(prof(3, [anchor, L1], his), A2, early), rwCP = checkpointOf(rw);
const r2a = read(prof(4, [anchor, L1, restA3], family), A3, rwCP), r2 = read(prof(5, [anchor, L1, exRest], his), EX, checkpointOf(r2a));
console.log('§3.4 — a recovery list is never overwritten\n');
console.log(`  checkpointed at version 2, holding 3 leaves at length 2; he republishes with a list of one: ${say(rw)}`);
console.log(`  the reader still holds ${rwCP.recoveryLists[2].leaves.length} leaves at length 2; his self-vouched restore: ${say(r2)}\n`);
assert.deepEqual([got(rw), rwCP.recoveryLists, r2a.verdict, got(r2)], [['ok', undefined], early.recoveryLists, 'ok', NOHOLD]);

// ---- rule 3: judged by the held list, never the carried copy ----
const rotCP = checkpointTo(prof(3, [anchor, L1, rotA3], family), A3);
const served = prof(4, [anchor, L1, exRest], his), r3 = read(served, EX, rotCP);
const heldList = rotCP.recoveryLists[2], carried = served.chain[2].recovery;
const adopted = adoptRecoveryLists({ ...rotCP.recoveryLists }, served, rotCP.chain.length);
const both = vouched(rotation(A2, EX, his), A2, [ex]);
const r3b = read(prof(4, [anchor, L1, both], his), EX, checkpointTo(prof(3, [anchor, L1, restA3], family), A3));
...
console.log('§3.4 — a majority at the divergence point, on exactly one side\n');
...
const older = read(prof(2, [anchor, L1], family), A2, checkpointed), twin = read(prof(3, [anchor, L1, restA3], family, 'Alice P.'), A3, checkpointed);
assert.deepEqual([got(older), got(twin)], [['contested', 'an older profile than the one this reader saw'], ['contested', 'two profiles at one version']]);
...
const first = await reader.read({ learned: A.x, at: OLD }), cold = first.checkpoint;
...
const after = await reader.read({ learned: A.x, at: NEW, checkpoint: cold });
await home.publish(2, { at: '2026-08-20T09:00:00Z', text: 'moved, and safe' });
await home.updateProfile(profileOf(3, [NEW]));
const now = await reader.read({ learned: A.x, at: NEW, checkpoint: after.checkpoint });
console.log('§3.5 — locations\n');
for (const [where, r] of [[OLD, first], [NEW, after], [NEW, now]]) console.log(`  read at ${new URL(where).host.padEnd(14)} version ${r.checkpoint.profileVersion}, names ${hosts(r.locations).padEnd(28)} remembered: ${hosts(r.checkpoint.locations)}`);
assert.deepEqual([first.anchor, after.anchor, now.locations, first.checkpoint.locations, after.checkpoint.locations, now.checkpoint.locations], [A.x, A.x, [NEW], [OLD], [OLD, NEW], [OLD, NEW]]);
...
await mumPub.publish(1, { at: '2026-08-20T11:00:00Z', rel: 'reply', target: { key: A.x, number: 2, hash: now.checkpoint.live.get(2), location: NEW }, text: 'welcome home' });
const mumFeed = await reader.read({ learned: mum.key.x, at: MUM });
const seen = new Map([[A.x, cold]]);                                    // sis: the checkpoint she took at pence.family, and nothing else
