...
// the split — the same bar §3.3 puts on every restore. Run: node examples/contest/contest.js
...
const family = commit([mum, sis, ex]), his = commit([ex]), pair = commit([mum, sis]);
...
const NOHOLD = ['identity', 'the chain of key changes does not hold'];   // §3.3 first: a link that is neither signed nor a majority is no link
const label = { [A.x]: 'anchor', [A2.x]: 'A2', [A3.x]: 'A3', [EX.x]: 'his key' };
const chainOf = (o) => o.chain.map((h) => label[h.key]).join(' → ').padEnd(22);
const size = (l) => `${l.leaves.length} leaves`;
...
console.log('  Judged by the list the reader holds, his one voucher of three is no link at all (§3.3).\n');
assert.deepEqual([got(rw), after.recoveryLists, r2a.verdict, got(r2)], [['ok', undefined], early.recoveryLists, 'ok', NOHOLD]);
...
console.log(`  against the list held      her link ${counts[0]} of 3   his link ${counts[1]} of 3   his is not a valid link`);
...
// The same rule where his link IS valid: signed with the A2 he holds, self-vouched, carrying his list of one.
...
console.log(`  and a valid link of his — a rotation with A2, vouched by himself, carrying his own list of one —\n  against her restore by mum and sis:  ${say(r3b)}  (1 of 3 under the held list, not the 1 of 1 the copy claims)\n`);
assert.deepEqual([counts, adopted[2], got(r3), got(r3b)], [[0, 1, 1], family, NOHOLD, HOST]);
...
const outcome = (r) => (r.verdict === 'host' ? 'host — his branch rejected' : r.why.startsWith('contested') ? 'identity — contested' : 'identity — his link does not hold');
for (const [mine, link] of [[restA3, exRot], [rotA3, exRot], [rotA3, exRest], [restA3, exRest], [restA3, coerced]]) {
  ...
  console.log(`  ${desc.get(mine).padEnd(30)} ${a}    ${b}    ${desc.get(link).padEnd(35)} ${outcome(r)}`);
  const valid = link.sig !== undefined || b * 2 > 3;                           // §3.3 comes before any contest
  assert.deepEqual(got(r), !valid ? NOHOLD : (a * 2 > 3) === (b * 2 > 3) ? TIE : HOST);
}
console.log('\n  A `sig` is not a vote: row 2 is two signed rotations, and each proves only that whoever');
console.log('  held A2 moved. He held it too. Rows 3 and 4 never reach the contest: one voucher of three');
console.log('  is not a valid link (§3.3). Row 5 is a majority on both sides — mum vouched for him.\n');
...
// The case the protocol exists for: the abuser is on the list, and he vouches for himself.
console.log('§3.3 / §3.6 — one bar, a majority: the case the protocol exists for\n');
console.log(`  the list                  ${size(family)} — mum, sis, and the ex. any two of them can bring her back`);
console.log('  her link                  a rotation she signed herself      0 vouchers');
console.log('  his link                  a restore he vouched for himself   1 voucher, his own — not more than half');
console.log(`  at a split                ${say(r3)}`);
const grab = read(prof(4, [anchor, L1, rotA3, restore(A3, EX, [ex], family)], family), EX, rotPin);
console.log(`  as an extension           he appends A3 → his key, vouched by himself alone:  ${say(grab)}`);
console.log('\n  A restore is valid only when more than half of the list vouches, so a listed member alone');
console.log('  can neither win a split nor quietly extend the chain to a key of his own. A lower threshold');
console.log('  here would be a second door into the identity that rule 4 never watches, which is why the');
console.log('  spec has no `k`: the majority is the one bar, for validity and for a contest alike.\n');
assert.deepEqual([got(r3), got(grab)], [NOHOLD, NOHOLD]);
...
const post = signFile({ n: 1, at: '2026-07-04T10:15:00Z', text: 'the peonies came back' }, A3);   // signed by A3 before any of this
const p5 = pinTo(prof(2, [anchor, L1p], pair), A2), c6 = read(prof(3, [anchor, L1p, hers], pair), A3, p5);
const p6b = pinTo(prof(4, [anchor, L1p, mended], pair), A3, p5), c6b = read(prof(5, [anchor, L1p, hisRot], pair), EX, p6b), keys = p6b.chain.map((h) => h.key);
console.log('§3.3 — the price of the majority rule, and the repair the single link shape buys\n');
console.log(`  a list of two, ${size(pair)} — mum and sis; the ex is not on this one`);
console.log(`  her link                a restore mum vouched alone   ${vouches(A2.x, hers, pair)} of 2 — not more than half`);
console.log(`  verdict                 ${say(c6)}`);
console.log('  She is stuck until a second member vouches. That is what the majority rule costs.\n');
console.log('  sis adds her voucher to the link alice already made (§3.3) and republishes at version 4:\n');
console.log(`  chain                   ${chainOf(prof(4, [anchor, L1p, mended], pair))}   the same link, now with two vouchers`);
console.log(`  her link                ${vouches(A2.x, mended, pair)} of 2 — a majority — and the ex rotating her old key now reads\n                          ${say(c6b)}`);
console.log(`  she never restored again: post 1, signed by A3 before all this, still verifies: ${verifyFile(post, keys) !== null}\n`);
assert.deepEqual([got(c6), keys, got(c6b), verifyFile(post, keys) !== null], [NOHOLD, [A.x, A2.x, A3.x], HOST, true]);
// The two limits, and the exit that runs through a person.
const coldRead = read(served, EX), cold = pinOf(coldRead), c7 = read(alice, A3, cold), wider = commit([mum, sis, ex, bro]);
...
assert.deepEqual([cold.recoveryLists[2], got(c7), edited.recoveryLists[2], onward.recoveryLists[3]], [his, NOHOLD, family, wider]);
