...
1. **No dependencies.** Implementable from a language's standard library, with no third-party
   packages. Ed25519, SHA-256, HTTP, JSON, and base64 are illustrations of what that means in
   practice, **not a closed list** — any primitive your target language's standard library actually
   provides is in bounds, and you should say which ones you are assuming. What is out of bounds is
   taking on a library or a piece of shared infrastructure: no JSON canonicalizer, no JOSE/COSE
   stack, no git binary, no blockchain, no DHT, no global registry or directory service.
