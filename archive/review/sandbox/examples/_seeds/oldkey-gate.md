...
**Question.** §3.3: a stolen old key "is closed without any revocation mechanism" because only the
current key signs the index. Two rules the sentence does not cover: §8.5 calls a file "the owner's"
when *any* chain key signed it; §4.2 says a number is issued once and a withdrawn one is never
...
withdrawal lines still in the index. Then the repaired fold as a pure function.

**Numbers** (stale if §8.5, §4.2, or the reader's fold change):
...
- As written, re-listing after his rewrite: pinned-before `ok`, **pinned-during `host: post 1 is
  ...
  his withdrawal lines still present: every reader `host: the index does not fold`. **Whether she
  may re-list depends on whether the thief happened to rewrite** — §4.2's "issued once" reaches
  only the index it is written in.
- Repaired fold (one hash per number, ever; the pin remembers the hash of what was withdrawn):
  ...
  index is signed by the current key alone (checked).

**Kill criterion.** The squat refused under §8.5 as written; a re-listing rule that lets a host
...
**Verdict.** §8.5 needs four words — *the key the chain currently ends on* — plus "or listed"; the
old-key squat is otherwise the exact harm §8.5 was written to stop, turned around. §4.2's rule
...
**Held to the ruling (2026-08-23, RULINGS §13.2).** The reader now carries the re-list rule and
...
pin is for); within one index the fold refuses the second hash for everyone. `hub.js` defaults to
`reclaim: 'current'`; the `'chain'` knob remains as the as-written demonstration.

**Run:** `node examples/_seeds/oldkey-gate.js`
