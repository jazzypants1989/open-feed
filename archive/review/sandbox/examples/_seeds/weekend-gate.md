...
**Candidate gate** (`../HANDOFF-to-spec.md` §1.H, and it answers §1.G on the way). Substrate: none.
...
key publishing. Then the hostile moves — withholding a listed post, serving an older index, swapping
...
- **Exactly three reader verdicts across all thirteen moments** (§1.G's target, measured, not
  counted from the rulings): `ok`, `host`, `identity`. `recently restored`, `withdrawn: 2`,
  `pending: n` and `no index I can verify` are **notes on an ok read**, not
...
1. **The index must be signed by the key that is current now**, so a rotation or a restore means
   writing the index *again*. This is not bookkeeping: it is the mechanism. A thief holding a
   rotated-out key can still sign a index, and the index is what admits posts — so if a reader
   accepted a index from any key in the chain, the thief would decide what counts as hers, and a
   restore would take nothing back from him. Written the other way round: **re-signing the index is
   what a restore actually restores.**

2. **A index that will not verify is not an accusation.** Between the two writes a rotation takes,
   an honest host is serving a index signed by a key the profile no longer ends on. The first version
   of this reader called that `host` — it accused an honest host of misbehaving during an honest
   rotation, in both write orders. The rule that works: a reader holding a index it verified itself
...
**Revert-checked** (`revert.js`, 6 rows): dropping the check that the profile's anchor is the key
this reader learned (the substituted-identity row goes red); accepting a index signed by any key in
...
**Verdict.** The weekend holds: 141 lines for a reader that follows key changes, folds a index,
...
**Run:** `node examples/_seeds/weekend-gate.js`
...
pending flag in the pin; `recovery-gate` put the fork-point recovery into the reader (+18 lines) and made
a restore link carry the list it satisfied; `media-gate` added the media file entry (+2). **The reader is
now 161 lines and the publisher 50.** The thirteen moments above still read the same, and the
"second profile at the same version" row is now settled by the recovery rather than by `version` equality
...
publisher gained the optional `name` of the spec §3.2. Publisher 51 → 50 lines; every claim above
