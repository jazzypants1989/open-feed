// A hub per open-feed-spec.md §8, shared by the review gates so three of them do not each carry
...
//   reclaim      'chain'   — §8.5 as first written: "the owner's file" is signed by ANY key in the chain
//                'current' — §8.5 as ruled 2026-08-23 (default): signed by the current key, or listed
//   verifyWrites false     — §8.5's "MAY check nothing on the ordinary path", applied to every file (as first written)
//                true      — §8.4 as ruled (default): refuse a profile whose chain does not walk or whose
//                            signature fails under its tip, and a index not signed by the current key
// The entity tag is the SHA-256 of the bytes served (§8.1 says a hub MAY do that; this one does).
...
// §3.3 as a hub would walk it: one link shape, every link carrying its recovery; valid by the previous
// key's signature or by enough distinct listed vouchers. Returns the chain keys or null.
export function walk(p) {
  if (!p || !Array.isArray(p.chain) || p.chain[0]?.key !== p.anchor) return null;
  for (let i = 1; i < p.chain.length; i++) {
    const link = p.chain[i], from = p.chain[i - 1].key;
    if (!Array.isArray(link?.recovery?.leaves)) return null;
    if (linkSig(from, link.key, from, link.sig)) continue;
    const leaves = new Set(link.recovery.leaves);
    const ok = new Set((link.vouchers ?? []).filter((v) => linkSig(from, link.key, v.key, v.sig) && leaves.has(H(Buffer.from(`${v.salt}|${v.key}`)))).map((v) => v.key));
    if (ok.size < link.recovery.k) return null;
  }
...
export class Hub {
  ...
  // key (as written) — or by the current key, or is what the index lists there (proposed).
  ownersFile(name, f, n) {
    ...
    const index = bodyOf(this.files.get(`${name}/index`) ?? Buffer.alloc(0));
    const listed = index?.entries?.some((e) => e[0] === n && e[1] === H(split(f).body));
  ...
  handle(method, url, b, ifMatch) {
    ...
    if (kind === 'profile' || kind === 'index') {                       // §8.1 compare-and-swap
      if (this.tag(key) !== ifMatch) return { status: 412, etag: this.tag(key) };
      if (kind === 'profile') {
        const o = bodyOf(b), old = cur && bodyOf(cur);
        if (!o) return { status: 400 };
        if (old && (old.anchor !== o.anchor || !(o.version > old.version))) return { status: 409 };   // §8.4
      ...
      this.files.set(key, b); return { status: 200, etag: H(b) };   // §8: 200 · 412, never 201
    }
    if (hash) {                                                         // §8.6 content-addressed twin
      if (cur && H(cur) === hash) return { status: 409 };
      if (cur && H(b) !== hash) return { status: 409 };
      this.files.set(key, b); return { status: cur ? 200 : 201 };
    }
    const n = +num;                                                     // §8.2 create-once, §8.5 reclaim
    if (cur) {
