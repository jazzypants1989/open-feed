...
**Candidate gate** (`../HANDOFF-final-review.md` §1.A; GOALS floor item 2, "still open" 2; rulings 8
...
**Today's construction, in five lines** (`src/enc.js`, spec §14): a JWE JSON serialization,
...
forbidden; the audience encrypted inside the plaintext, never in a header; carrier binding by
...
run a encrypted post through the unchanged reader on this substrate. "No new signing construction"
...
post's author anchor key and number; the audience is the first member of the encrypted plaintext;
...
`src/enc.js`; and over a loopback socket, Alice's encrypted posts 5 and 6 published by
...
- Over the socket: Alice reads `ok`, two posts, the `encrypted` field an opaque object; Mum opens post
  5 after the reader returned; the host opens nothing. The signature line of a encrypted post is 86
...
§14.2.1 existed for is alive on this substrate, and `SKETCH.md` §8 saying "§14's carrier binding
...
anchor key and the post's number; an envelope lifted into another post does not open.** **A
...
**Run:** `node examples/_seeds/envelope-gate.js`

**Changed while writing the spec (2026-08-21).** The construction moved out of this gate into
`gates/envelope.js`, unchanged, so that `tools/regen.js` generates Appendix B's encrypted vector from
