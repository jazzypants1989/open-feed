...
**Question.** §6.4 seals the audience inside "so a recipient learns who else can answer", as a
...
**Numbers** (stale if §6.4, §3.8 or `envelope.js` change):
...
- Under §3.8 he seals to **2 of 3**; Sis opens his reply and gets **nothing** — the thread split
  §6.4 says the inside audience exists to prevent, with no error anywhere. Sealing to the audience
...
**Kill criterion.** The replier able to reach the third member's profile from what §6.4 gives him
...
**Verdict.** §6.4's audience must name people, not keys: `{key, read, at}`, exactly §5.4's
