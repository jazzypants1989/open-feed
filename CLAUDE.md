...
| `tmp/check-prototypes.js` | `npm run prototypes`. Runs every gate in `tmp/prototypes/` and fails if any no longer holds. **The full suite can cost minutes** and writes `tmp/prototype-results.json` (gitignored) every run — read that before paying for a rerun, and never edit `src/`, the spec, or a prototype while a run is in flight: a half-landed edit reads as a failure. It exists because the convention decayed silently: `src/` drifted out from under three prototypes, one had been exiting 1 for several commits, and the old handoff went on arguing §7.6's case from a number that no longer reproduced. **A prototype nobody re-runs is a claim, not evidence** |
| `tmp/archive/` · `tmp/prototypes/` · `tmp/measure/` | The Cutting Campaign's layout (`PLAN.md`, Stage 1): experiment originals verbatim in `archive/` (paths fixed, runnable by hand, never run by CI); clean **gates** + **verdict cards** in `prototypes/` (contract in its README); informative measurement scripts in `measure/`. The per-prototype rows below describe archived originals whose paths are now `tmp/archive/<name>-prototype.js`; each row is deleted as its card lands, and the whole pile goes when Stage 1 closes |
...
| `tmp/delivery-chain-prototype.js` | ADOPTED (§10.6). Prices the delivered column's integrity gap and the two mechanisms rejected before it — a published receipt map (the hub signs it; and a public receipt for a private message is a worse disclosure than the drop) and a sender-side commitment (cannot detect what the recipient never knew to expect). Q4 is the load-bearing one, and its answer is §11.2's one-recipient rule: every placement for per-pair state inside one multi-recipient signed object either broadcasts the audience or — the JWE-header variant, whose slot is cleartext apart from the blinded tag — re-links it, so the multi-recipient delivered item is forbidden rather than accommodated |
