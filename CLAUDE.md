...
| `tmp/check-prototypes.js` | `npm run prototypes`. Runs every `*-prototype.js` and fails if any no longer holds. **The full suite costs minutes** and writes `tmp/prototype-results.json` (gitignored) every run — read that before paying for a rerun, and never edit `src/`, the spec, or a prototype while a run is in flight: several prototypes read those files mid-run and a half-landed edit reads as a failure. It exists because the convention decayed silently: `src/` drifted out from under three of them, one had been exiting 1 for several commits, and `HANDOFF.md` went on arguing §7.6's case from a number that no longer reproduced. **A prototype nobody re-runs is a claim, not evidence** |
