...
| `src/` | **Reference implementation**, zero dependencies: Level 1 verifier and Level 2 publisher. `canonical.js` is RFC 8785 + a hand-written I-JSON parser, because §6.3's duplicate-member rejection is not something `JSON.parse` can do; `jws.js` is the §6 construction; `chain.js` is §5.3's walk for both chained documents; `manifest.js` is §9.3; `migration.js` is §3.4 — predecessor equivalence held once and asked by every site that needs it, plus the retained predecessor *document* a recovery co-signature resolves in, which a pin alone cannot supply; `publish.js` emits every artifact as bytes; `reader.js` composes the lot into the Level 1 consumer, and the *order* is its content — identity chain before manifest, manifest before items; `cli.js` is that reader as a command. **`fetch.js` is the only module that opens a socket — keep it that way.** Node's `crypto` has Ed25519 natively — no `jose`, no `@noble`, no `canonicalize` |
