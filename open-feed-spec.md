...
**Where the field lives, because one signed item may reach several inboxes.** For an **audience of one** — which §11.2 says is the whole of the cleartext delivered column, since "there is exactly one counterparty" — `_delivery` sits at the top level and names no recipient at all. For an item wrapped to several recipients (§15.2), it goes in that recipient's own per-recipient JWE header beside `_tag`, which §15.2 has already built and already blinded. Two placements, one field, and both at points where the protocol already splits per recipient. The alternatives are worse in ways worth recording so they are not proposed again: a top-level array naming each recipient tells every recipient's host who else was written to, which is the disclosure §11.4 spends its length avoiding; and one item per recipient breaks the `(author, id, _version)` identity that §7.3, §7.5, and §10.3 all depend on.
