...
> A receiver MUST NOT place a delivered-only item into any publicly-readable artifact: not a feed (§7.1) — its own or an aggregate — not a manifest (§9), not a replies-endpoint response (§16.3), and not a gateway emission to a foreign network (§7.5, Appendix E).
