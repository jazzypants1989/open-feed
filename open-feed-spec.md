...
- **The member name `__proto__`**, which this specification reserves and no document may carry, in a header or anywhere else. RFC 7493 does not exclude it and JavaScript makes it a hazard that fails *open*: assigning it while rebuilding a parsed object sets a prototype rather than creating a member, so the value vanishes from the serialization — and therefore from the signature payload, the manifest commitment, and every pin — while staying visible to every later read of that object. Appended to somebody else's *already-signed* item it survives every check defined here, which is worse than a forgery because nothing is forged. The name has no legitimate use in this protocol, and rejecting it is the one response that requires no care anywhere else.

**A chained document MUST arrive as its own canonicalization.** §5.4 obliges a producer to serve retained versions byte-identically; this is the consumer's half, and without it neither that rule nor §5.1's "full published canonical bytes" is ever actually checked — a verifier that re-canonicalizes whatever it parsed pins a normalization of what it was served rather than the bytes it was served, and every remaining parser divergence lives in that gap. A consumer MUST reject an identity document or a manifest whose body is not byte-identical to the canonicalization of the value parsed from it. Feeds are excluded and cannot be included: a feed is neither signed nor chained, and its items are covered by the rule above instead.
