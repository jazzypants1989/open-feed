...
- **Numbers outside I-JSON's interoperable range.** RFC 8785's number serialization is defined on IEEE-754 doubles, so a number that is not exactly one — an integer beyond ±(2⁵³−1), or a decimal only a wider type preserves — parses to *different values* in implementations with different number types, and the hash forks with no other symptom: a bignum-equipped verifier and a double-equipped one each compute a signature payload the other never sees. RFC 7493 makes the double bound a SHOULD; here the serialization is the signature payload, so producers MUST NOT emit such a number in any signed JSON, and a verifier whose parse cannot preserve a number token's value MUST reject the document rather than proceed on the lossy reading. The test is the implementation's own round trip — a token whose parsed value re-serializes to a different mathematical value was lossy — which is the same parse-serialize pair the other rejections already require.
...
### 7.6. Derived Item URLs (OPTIONAL)

A feed serves its items in pages; a manifest names each one's exact bytes. Nothing lets a consumer ask for **one item**, and §9.3's withholding verdict is the rule that suffers for it — the only honest way to assert "committed but never yielded" is a complete pass over the feed, which §13.4 budgets nothing for and which a paginated reader cannot make. A producer MAY therefore also serve each committed revision individually:
...
- **Consumers MUST NOT require it.** A feed without derived item URLs is fully conformant and is read exactly as before; what its readers lose is the ability to assert withholding about anything (§9.3).
...
Note what that leaves a consumer of a feed with no derived item URLs: a verdict it can reach only by reading the whole catalog on every poll, which against a decade of family posting is several megabytes to say something about items nobody suspects. **The state is therefore close to unreachable on the pull path unless the publisher offers §7.6**, and that is the honest reason §7.6 exists rather than a convenience. It is still OPTIONAL, and a consumer that cannot obtain an item reports it as not yet seen — the safe reading, and the one that accuses nobody.
...
- It carries no authority and grants no one anything. It is the author's statement about who they wrapped to, and §15.5.1 already establishes that no such statement is checkable by anyone.
