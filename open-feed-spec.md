...
**16.2.2. Recovery propagation.** A recovery-based migration (§3.4) cannot publish a `successor` from the lost domain, so consumers who lost the old pointer may never learn of it. A trusted peer's pin naming a `seq` for the old identity **beyond** the consumer's own pin is the signal to re-walk: the consumer fetches that identity's history, walks to the peer's `(seq, hash)`, and discovers the `predecessor` and recovery co-signature there. The pin says "look again"; verification remains the consumer's own.
...
An implementation that skips this check converts every delivered-private interaction it receives into a public one, silently and by default. §15.4 routes group interactions down the delivered path precisely to keep a reply graph off the public web, and that design depends entirely on this rule holding here.
...
|----------|---------------------|------------------|
| Identity document, manifest, retained prior versions, follows, pins, export bundle, inbox body | `application/json` | any JSON; reject non-JSON |
