...
- `attachments` SHOULD inline the referenced bytes. Where one JSON document makes that impractical — a decade of photographs base64-encoded is not something either side can stream — the bundle MAY instead be delivered as an **archive container**: any ordinary archive format whose entry `openfeed-export.json` is the document above and whose remaining entries are the attachment bytes, each named by its `_sha256`. That is still one bundle and still self-verifying, because the hash naming each file is the one inside the signed item that references it. Only where neither form is possible may the bundle fall back to retaining each `url` and `_sha256` alone — and that fallback points at the host being left, so it is a degraded export rather than an equivalent one. An export that omits the photos has not exported a family archive.
...
- It MUST appear inside the **sealed plaintext** and MUST NOT appear in a JWE per-recipient header, which §15.2 keeps untagged so that an *observer* learns nothing. Readers learning the audience is the point; observers learning it is the leak that field exists to prevent.
