...
**Re-signing the back catalog.** Migration makes the new feed canonical for *new* content, but previously-published items carry the old feed's URL in their signed `_feed_url`, so at the new home they would be mere copies (§7.5). The procedure therefore ends with bulk re-signing: republish the back catalog at the new feed — same `id`s, bumped `_version`, `date_modified` set to the re-signing time, `_feed_url` naming the new feed — and commit it in the new feed's manifest. A verified migration is the sole exception to the id/feed binding rule (§7.5): the binding follows the identity to its successor feed.
...
Step 5 exists because §13.2's transparency claim assumes an auditor. Equivocation is *detectable* only by someone who compares, and the party with the strongest interest in comparing is the identity itself, which cannot compare without a record of what it actually published — where a host holds the signing key on the user's behalf, this record is the user's only means of noticing a version they did not ask for. It is not sufficient alone, since a host that knows which client belongs to the owner can serve that client the honest branch, so the durable check remains cross-observer comparison by *other people* (§5.3.1, §16).
...
- A consumer MUST verify an item's `_feed_url` matches the feed URL it was fetched from (after normalization) before treating it as canonical. A mismatch marks the item a copy: display it, attribute it to its signer, but do not grant it liveness or manifest standing.
...
- An `id` is permanently bound to a single `_feed_url`. The same `id` MUST NOT be signed with two different `_feed_url` values: the bytes would differ while `(author, id, _version)` claims to name one exact revision (§7.3), and inbox dedup (§10.3) would silently drop one variant. Cross-posting the same content to another feed uses a **new item** with a fresh `id` carrying a `repost` or `quote` relation to the original. A verified migration is the sole exception (§3.4).
...
**`type`** is a registered token from the table **or** an absolute URL for custom relations, per the token-vocabulary rule (§2.1). **`to`** is a single target URI: for items, `{feed_url}#{item_id}` (RECOMMENDED — receivers resolve relevance structurally by splitting at the last `#`, unambiguous because ids never contain `#`, §7.2) or the target's permalink `url`, which forces receivers to recognize their own permalinks; for `mention`, the mentioned identity URL. Multiplicity is expressed with **multiple entries**, never an array in `to`, so a reply that mentions two people is three entries. Entries are **open objects** whose unknown keys MUST be preserved — this is where per-relation extension and bridge round-trip data live, such as `_emoji` on a `like`.

Custom-typed entries follow the same shape; receivers store unknown types and MAY hide them. Clients SHOULD NOT render content-less relation items as posts; they are activity. Publishers SHOULD segregate them into a separate **activity feed** — a `feeds` entry with `rel: "activity"` (§3.2.1) — which keeps the primary feed clean for Level 0 readers that would otherwise render bare likes as posts, and keeps the primary manifest from being dominated by likes. An activity feed is manifested like any other; §9.2's scheduled cadence is what keeps its chain from advancing once per reaction.

Interaction items live in their author's feed like any other item (SHOULD — they MAY be inbox-only, in which case `_feed_url` is omitted). One object, one signature: publishing and delivering are the same bytes, so there is nothing to keep in sync.
...
The cheapest interoperability is not a bridge, because Open Feed's wire formats are already other people's wire formats; README describes that route, which needs nothing in this specification implemented. This appendix governs the expensive route.
