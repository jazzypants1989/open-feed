...
`audience` is an OPTIONAL array of identity URLs naming who the author wrapped this item to. It exists for one case the rest of this specification cannot otherwise reach: **a reply to encrypted content that the other recipients can also read.** §11.2 permits an author to broadcast to a list only they hold, and is emphatic that this is not a group — because a replier cannot wrap to a list they cannot read, so every reply comes back to the author alone. A declared audience is that list, carried where the audience can read it and nowhere else.
