...
// rumor rule. Most scenarios run twice — cold and pinned — because pinning is what §7 is about.
...
test('§7 a cold read, a pinned read, a withdrawal, a rewrite, a re-listing', async () => {
  const s = await scene();
...
test('§7.2 the index: withholding, a rollback, a swap, and the rotation window', async () => {
  ...
  assert.equal(view(await s.read()), 'host: no index served');
  assert.equal(view(await s.read(good.pin)), 'ok [1,2,3] no index I can verify', 'a pinned reader keeps the index it verified');
...
test('§4.4 / §7.2 a rotation: the index is re-signed under the new key; in between, a pinned reader notes and a cold one retries', async () => {
  ...
  assert.equal(view(await s.read(before.pin)), 'ok [1,2,3] no index I can verify');
...
test('§3.4 / §7.3 a restore: "recently restored" is a note for seven days of the reader\'s clock, never a verdict', async () => {
  ...
  // §3.3: a restore that also drops her name, or her location, is refused by a pinned reader.
