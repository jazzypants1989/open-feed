...
test('§3.6 rule 1: a served chain must extend the pin key for key; a prefix at a higher version is a split', () => {
...
test('§3.6 rule 2–3: recoveryLists are held at every length from the first read, and a carried recovery at a held length is ignored', () => {
  ...
  assert.deepEqual(Object.keys(pin.recoveryLists), ['1', '2', '3']);
...
test('§3.3 a restore changes the key and nothing else — checked by a pinned reader', () => {
  const pin = pinOf(prof(3, honest, K3));
