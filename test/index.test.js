// §4 — the index: replay, one hash per number, media, highest, the checkpointed checks, the rewrite.
import test from 'node:test';
import assert from 'node:assert/strict';
import { replay, checkIndex, checkAgainstCheckpoint, liveEntries } from '../src/index.js';
...
test('§4.2 replay: admit, withdraw, re-list at the identical hash only', () => {
  ...
  assert.equal(replay([[1, 'a'], [1, 'a']]), null, 'a live number listed twice');
  assert.equal(replay([[2, null]]), null, 'a withdrawal of nothing');
  assert.equal(replay([[0, 'a']]), null); assert.equal(replay([[1.5, 'a']]), null); assert.equal(replay([[1, 'a', 'pending']]), null, 'the retired three-element line');
  assert.equal(replay([[1, 'a'], [1, null], [1, null]]), null, 'withdrawn twice');
  assert.equal(replay([[1, 'a'], [1, null], [1, 'a'], [1, null]]).live.size, 0);
...
test('§4.4 media: listed by hash, new whenever they appear, withdrawn by [hash, null]', () => {
  ...
  assert.deepEqual(live(replay([['h1'], ['h1', null], ['h1']])), ['h1'], 'a withdrawn media file may return');
...
test('§7.2 step 9 against a checkpoint: no rollback, no insertion below highest, no change, withdrawn hashes remembered', () => {
  ...
  assert.deepEqual(live(replay([[1, 'a'], [3, 'c'], ['p']])), [1, 3, 'p']);
