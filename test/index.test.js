// §4 — the index: the fold, one hash per number, media, highest, the pinned checks, the rewrite.
...
test('§4.2 the fold: admit, withdraw, re-list at the identical hash only', () => {
  ...
  assert.equal(fold([[1, 'a'], [2, 'b'], [2, null]]).highest, 2, 'highest counts what was issued');
});
...
test('§4 / §4.3 the index\'s shape: entries first, version and highest integers, highest at or above the highest number', () => {
  ...
  assert.match(checkIndex({ entries: [[3, 'c']], version: 1, highest: 2 }, set), /highest/);
  assert.match(checkIndex({ entries: [[3, 'c']], version: -1, highest: 3 }, set), /version/);
  assert.equal(checkIndex({ entries: [[3, 'c']], version: 1, highest: 7 }, set), null, 'highest may run ahead of what is listed');
});

test('§7.2 step 9 against a pin: no rollback, no insertion below highest, no change, withdrawn hashes remembered', () => {
  const pin = { indexVersion: 2, indexHash: 'H2', highest: 3, live: new Map([[1, 'a'], [3, 'c']]), withdrawn: new Map([[2, 'b']]) };
  const ok = (entries, version = 3, highest = 3, address = 'H3') => checkAgainstPin({ obj: { version, highest }, address }, fold(entries), pin);
  ...
  assert.equal(checkAgainstPin({ obj: { version: 3, highest: 3 }, address: 'H3' }, fold([[1, 'a'], [3, 'c']]), { ...pin, withdrawn: new Map(), live: new Map([[1, 'a'], [3, 'c']]) }).notes.length, 0);
  assert.equal(checkAgainstPin({ obj: { version: 3, highest: 3 }, address: 'H3' }, fold([[1, 'a'], [2, 'b'], [3, 'c']]), { ...pin, withdrawn: new Map() }).why, 'post 2 is listed now and was not before');
  const r = ok([[3, 'c']]);
...
test('§4.5 a rewrite keeps the live set, in order, and drops what withdrawals left behind', () => {
